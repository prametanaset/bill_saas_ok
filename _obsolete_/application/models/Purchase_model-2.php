...
class Purchase_model extends MY_Model {
	...
	public function verify_save_and_update(){
		...
		if($amount>0 && !empty($payment_type)){
			...
			if($amount>$tot_total_amt){
				echo "ยอดเงินที่ต้องชำระไม่ควรเกินยอดเงินในใบแจ้งหนี้!!";exit;
			}
			...
			*/
			if($command=='update'){
				...
				if(($tot_payment+$amount)>$tot_total_amt){
					echo "ยอดเงินที่ต้องชำระไม่ควรเกินยอดเงินในใบแจ้งหนี้!!\n กรุณาตรวจสอบการชำระเงินก่อนหน้าด้วย.";exit;
				}
			}
	...
	public function delete_purchase($ids){
		...
		if($q8->num_rows()>0){
			$i=1;
			echo "ขออภัย! บันทึกไม่ถูกลบ! พบใบส่งคืนสินค้า จากการซื้อสินค้า:";
			foreach($q8->result() as $res){
				echo "<br>".$i++.".Return Invoice Against Purchase Id:".$res->purchase_code;
			}
			echo "<br>หากต้องการลบการซื้อ! คุณต้องลบการส่งคืนสินค้า!";
			exit();
		}
		  ...
		  <div class="modal-dialog ">
		    ...
		    <div class="row">
		      	...
		      	<div class="row invoice-info">
			        <div class="col-sm-4 invoice-col">
			          ...
			          <address>
			            ...
			            <?php echo (!empty(trim($supplier_gst_no))) ? $this->lang->line('gst_number').": ".$supplier_gst_no." " : '';?>
			            <?php echo (!empty(trim($supplier_tax_number))) ? $this->lang->line('tax_number').": ".$supplier_tax_number."<br>" : '';?><br>
			          </address>
			        </div>
			        <!-- /.col -->
			        <div class="col-sm-4 invoice-col">
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" onclick="save_payment(<?=$purchase_id;?>)" class="btn bg-primary btn-lg place_order btn-lg payment_save">Save<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>

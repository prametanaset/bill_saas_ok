        ...
        <div class="col-xs-12 table-responsive">
          <table class="table  records_table table-bordered">
            ...
            <tr>
              ...
              <th class='text-center'><?= $this->lang->line('before_discount'); ?></th>
            ...
            <tbody>
             ...
             <?php
              ...
              foreach ($q2->result() as $res2) {
                  ...
                  echo "<td class='text-right'>".store_number_format($res2->tax)."% "  . $res2->tax_name . "</td>";          
           ...
           //----------------------------------------------------------------------------//
                 ...
                 $tax_no = ($total_in_tax -  $tot_tax_amt ) /100 ; //สินค้ารวม VAT - vat(ก่อนลดทั้งหมด) / 100 =  VAT 1%
                 $vat_no = ($tax_no != 0) ? $tot_tax_amt / $tax_no : 0; // VAT %  (จำนวนเงิน VAT / VAT 1% : เช่น = 7 % )

                 $vat_no = number_format(round($vat_no, 1), 1, '.', '');
                 //-----------------------------------//

                $percent_all = ($tot_total_cost + $other_charges_amt)  / 100 ; // คำนวน มูลค่ารวม 1 %   -----(ก่อนหักส่วนลด)          
                $disc_per = $tot_discount_to_all_amt / $percent_all;        // คำนวนส่วนลดเป็นเปอร์เซ็นต์  
                //-----------------------------------//

                $non_tax_all =  ($total_non_tax / 100) *  $disc_per;
              


                $total_taxable = $total_in_tax + $other_charges_amt ;  // จำนวนเงินรวมภาษี + ค่าบริการ
                $disc_taxable = ($total_taxable / 100) * $disc_per ;  // จำนวนเงินรวมภาษี + ค่าบริการ - ส่วนลดทั้งบิล %
                $taxable = $total_taxable - $disc_taxable ;  //มูลค่ารวมภาษี    -----(หลังส่วนลด)

              
                $vat = $taxable / ( 100 + $vat_no);   // จำนวนเงิน vat 1%   -----(หลังหักส่วนลด)
                $vat_amt  = $vat *  $vat_no ;      // มูลค่าภาษีมูลค่าเพิ่ม                
                                                                                        
                $total_vatable =  $taxable - $vat_amt ;   //  มูลค่าคำนวน VAT
                   
                 $total_tax_zero_amt = $grand_total -  ($total_vatable + $vat_amt) ;  // มูลค่าสินค้ายกเว้นVAT  - ส่วนลดทั้งบิล   ---- (หลังส่วนลด)
                  $total_tax_zero_amt = is_nan($total_tax_zero_amt) ? 0 : $total_tax_zero_amt; // เปลี่ยนค่า nan เป็น 0
              }
              ?>
      ...
      <div class="row">
       <div class="col-md-6">
          ...
          <div class="row">
              <div class="col-md-12">
                 <div class="form-group">
                    <label for="sales_note" class="col-sm-4 control-label" style="font-size: 17px;"><?= $this->lang->line('saleman'); ?></label>    
                    <div class="col-sm-8">
                   ...
                   <!--   <?php if(!empty($tot_price_zero_vat)) {?>
                       <tr>
                          ...
                          <th class="text-right" style="padding-left:10%;font-size: 11px;">
                             <h3><b id="subtotal_amt" name="subtotal_amt"><?php echo store_number_format($total_non_tax);?></b> </h3>
                          </th>
                       </tr>
                       <?php } ?> 
                      ...
                      <?php if(!empty($total_in_tax !=0)) {?>
                       <tr class=''>
                        ...
                        <?php if(!empty($other_charges_amt !=0)) {?>
                          ...
                          <th class="text-right" style="padding-left:10%;font-size: 17px;">
                             <h3><b id="other_charges_amt" name="other_charges_amt"><?=store_number_format($other_charges_amt);?></b></h4>
                          </th>
                         </tr> 
                          <?php } ?>       
                         ...
                         <tr>
                          ...
                          <th class="text-right" style="padding-left:10%;font-size: 17px;">
                             <h3><b id="discount_to_all_amt" name="discount_to_all_amt"><?=store_number_format($tot_discount_to_all_amt);?></b> </h3>
                          </th>
                         </tr>  
                         <?php } ?> 
                  

                         <?php if(!empty($total_tax_zero_amt !=0)) {?>
                       <tr class='text-primary'>
                          <th class="text-right" style="font-size: 17px;"><b><?= $this->lang->line('total_zero_vat_amt') ; ?></b></th>
                            <th class="text-right" style="padding-left:10%;font-size: 17px;">
                            <h3><b id="total_amt" name="total_amt"><?=store_number_format( $total_tax_zero_amt);?></b>  </h3>
                            <th>
                         </th>
                        </tr>
                       <?php } ?> 
                    ...
                    <?php if(!empty($total_in_tax !=0)) {?>
                       <tr class='text-primary'>
                          <th class="text-right" style="font-size: 17px;"><b><?= $this->lang->line('before_amt') ; ?></b></th>
                            <th class="text-right" style="padding-left:10%;font-size: 17px;">
                            <h3><b id="total_amt" name="total_amt"><?=store_number_format( $total_vatable);?></b> </h3>
                            <th>
                         </th>
                        </tr>
                       <?php } ?> 
                     ...
                     <?php if(!empty($total_in_tax !=0)) {?>
                       <tr class='text-primary'>
                          <th class="text-right" style="font-size: 17px;"><b><?= $this->lang->line('vat_amount') .( $vat_no)." % "; ?></b></th>
                            <th class="text-right" style="padding-left:10%;font-size: 17px;">
                            <h3><b id="total_amt" name="total_amt"><?=store_number_format( $vat_amt );?></b> </h3>
                            <th>
                         </th>
                        </tr>
                      <?php } ?> 
      ...
      <div class="row no-print">
       <div class="col-xs-12" style="margin-bottom:5px">
        <div class="col-xs-6">
          ...
          <a href="<?php echo $base_url; ?><?=$str2;?><?php echo  $sales_id ?>"target="_blank" class="btn btn-info">
            <i class="fa  fa-edit"></i> แก้ไข
          </a>
         <?php } ?>
                
         <?php if($CI->permissions('sales_return_add')) { ?>
            <a href="<?php echo $base_url; ?>sales_return/add/<?php echo  $sales_id ?>"target="_blank" class="btn btn-danger">
            <i class="fa  fa-undo"></i> ลดหนี้/คืนสินค้า
          </a>
          <?php } ?>
       ...
       <!--
        <a onclick="printTemplate('<?php echo $base_url; ?>pos/print_invoice_pos/<?php echo  $sales_id ?>')" target="_blank" class="btn btn-info">
         ...
         <a href="<?php echo $base_url; ?>pdf/sales/<?php echo  $sales_id ?>" target="_blank" class="btn btn-success">
            <i class="fa fa-file-pdf-o"></i> 
          ใบกำกับภาษี PDF
        </a>-->
         <a onclick="printTemplate('<?php echo $base_url; ?>pos/print_invoice_pos/<?php echo  $sales_id ?>')" target="_blank" class="btn btn-success">
            <i class="fa fa-file-text"></i> 
          ใบเสร็จPOS
        </a>
        ...
        <!--  -->
         <a href="<?php echo $base_url; ?>pdf_facturx/sales/<?php echo  $sales_id ?>" target="_blank" class="btn btn-primary">
            <i class="fa fa-file-pdf-o"></i> 
               ใบแจ้งหนี้ PDF/A-3b
         </a>
       <!-- <a href="<?php echo $base_url; ?>sales/print_invoice/<?php echo  $sales_id ?>" target="_blank" class="btn btn-primary">
            <i class="fa fa-file-pdf-o"></i> 
               พิมพ์ใบกำกับภาษี
         </a>  -->            		  
        </div>     

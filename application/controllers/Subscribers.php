...
class Subscribers extends MY_Controller {
	...
	public function save_update_subscription(){
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo "กรุณากรอกข้อมูลในช่องที่จำเป็นต้องระบุ ( * ) .";
		}
	}
	...
	public function ajax_list()
	{
		...
		foreach ($list as $subscription) {
			...
			$row[] = ($subscription->max_warehouses == -1 || $subscription->max_warehouses == '∞') ? 'Unlimited' : number_format($subscription->max_warehouses);
			$row[] = ($subscription->max_users == -1 || $subscription->max_users == '∞') ? 'Unlimited' : number_format($subscription->max_users);
			$row[] = ($subscription->max_items == -1 || $subscription->max_items == '∞') ? 'Unlimited' : number_format($subscription->max_items);
			$row[] = ($subscription->max_invoices == -1 || $subscription->max_invoices == '∞') ? 'Unlimited' : number_format($subscription->max_invoices);

					$str = "<b>ชำระเงิน:</b> ".$subscription->payment_gross;
					$str .= "<br><b>ช่องทางชำระเงิน:</b> ".$subscription->payment_type;
					$str .= "<br><b>สถานะการชำระเงิน:</b> ".$subscription->payment_status;
			$row[] = $str;

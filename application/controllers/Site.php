...
class Site extends MY_Controller {
	...
	public function index(){
        ...
        // Load data for System tab
		$data['timezone_list'] = $this->db->where('status', 1)->get('db_timezone')->result();
		$data['currency_list'] = $this->db->where('status', 1)->get('db_currency')->result();
		$data['language_list'] = $this->db->where('status', 1)->get('db_languages')->result();

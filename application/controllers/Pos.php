...
class Pos extends MY_Controller {
	...
	public function edit($sales_id){
		...
		if ($this->db->field_exists('reference_type', 'db_sales')) {
			$sales_rec = $this->db->select('reference_type')->where('id', $sales_id)->get('db_sales')->row();
        ...
        if($this->input->get('sell_again')){
            $data['sell_again_id']=$sales_id;
            $data['page_title']='ทำรายการซ้ำ (Sell Again)';
            
            // Generate NEW invoice codes
            $init_code=get_only_init_code('sales');
            $count_id=get_last_count_id('db_sales');
            $customer_id = get_sales_details($sales_id)->customer_id;
        } else {

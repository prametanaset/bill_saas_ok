...
class Payment_types extends MY_Controller {
	...
	public function new_payment_type(){
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo "กรุณาระบุชื่อประเภทการชำระเงิน.";
		}
	}
	...
	public function ajax_list()
	{
		...
		foreach ($list as $payment_type) {
			...
			$row[] = $str;			
			         $str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											if($this->permissions('payment_types_edit') || is_admin())
											$str2.='<li>
											...
											if($this->permissions('payment_types_delete') || is_admin())
											$str2.='<li>
			...
			$row[] = ((strtoupper($payment_type->payment_type)==strtoupper(cheque_name())) || (strtoupper($payment_type->payment_type)==strtoupper(cash_name())) || (strtoupper($payment_type->payment_type)==strtoupper(card_name()))) ? '-NA-' : $str2;

...
class Expense extends MY_Controller {
				...
				$row[] = ($expense->created_by);			
				     ...
				     $str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											<li>
												<a target="_blank" title="Print Voucher ?" href="'.base_url().'expense/print_expense_voucher/'.$expense->id.'">
													<i class="fa fa-fw fa-print text-blue"></i>พิมพ์ใบสำคัญจ่าย
												</a>
											</li>
	...
	public function print_expense_voucher($expense_id){
		if(!$this->permissions('expense_view')){
			$this->show_access_denied_page();
		}
		$expense_details = $this->db->query("select * from db_expense where id=$expense_id")->row();
        $category_details = $this->db->query("select * from db_expense_category where id=".$expense_details->category_id)->row();
        $store_details = $this->db->query("select * from db_store where id=".$expense_details->store_id)->row();

        // Construct Custom Voucher No
        // Prefix from db_store (expense_init) - referring to db_prefixes logic via db_store or custom_helper
        // Actually custom_helper shows expense_init is in db_store.
        $expense_prefix = $store_details->expense_init;
        $expense_number = str_pad($expense_details->count_id, 4, '0', STR_PAD_LEFT);
        $voucher_no = $expense_prefix . "-" . $expense_number;
		...
		$this->load->view('print-expense-voucher',$data);
	}
}

...
class Account_transactions extends MY_Controller {
	...
	public function get_debit_code($debitpayment_id){
		if(empty($debitpayment_id)) { return ''; }
		return $this->db->select("a.debit_code,a.id")
						->from("db_salesdebit a")
						->from("db_salespaymentsdebit b")
						->where("a.id=b.debit_id")
						->where("b.id=",$debitpayment_id)->get()->row();
	}
	public function ajax_list()
	{
		...
		if($from_date != '1970-01-01'){
			// CASE 1: Filtered by date - sum everything before from_date
			$query = $this->db->select("COALESCE(SUM(credit_amt),0)-COALESCE(SUM(debit_amt),0) as balance")
							  ->from("ac_transactions")
							  ->where("store_id",get_current_store_id())
							  ->where("(credit_account_id=$account_id or debit_account_id=$account_id)")
							  ->where("transaction_date <", $from_date)
							  ->get();
			$prev_balance = $query->row()->balance;
		}
		else if($_POST['start'] > 0){
			// CASE 2: No date filter but on Page 2+ - sum everything on previous pages
			$start = $_POST['start'];
			$sub_query = $this->db->select("credit_amt, debit_amt")
								  ->from("ac_transactions")
								  ->where("store_id",get_current_store_id())
								  ->where("(credit_account_id=$account_id or debit_account_id=$account_id)")
			...
			$query = $this->db->query("SELECT COALESCE(SUM(credit_amt),0)-COALESCE(SUM(debit_amt),0) as balance FROM ($sub_query) as prev_tx");
			$prev_balance = $query->row()->balance;
		}
		...
		foreach ($list as $accounts) {
			...
			$row[] = show_date($accounts->transaction_date);
				...
				if($accounts->transaction_type!='OPENING BALANCE PAID'){
					...
					if(!empty($accounts->supplier_id)){
						...
						else if(!empty($accounts->ref_salespaymentsdebit_id)){
							$query = $this->get_debit_code($accounts->ref_salespaymentsdebit_id);
							$debit_code = $query->debit_code;
							$debit_id = $query->id;
							$from_ = "<a data-toggle='tooltip' title='View Sales Debit Payments' href='".base_url('sales_debit/invoice/').$debit_id."'>".$debit_code."</a>";
						}
					}
					else{
					...
					if(!empty($accounts->supplier_id)){
						...
						if(!empty($accounts->ref_salespayments_id)){
							...
							$debit_code = $query->debit_code;
							$debit_id = $query->id;
							$to_ = "<a data-toggle='tooltip' title='View Sales Debit Payments' href='".base_url('sales_debit/invoice/').$debit_id."'>".$debit_code."</a>";
						}
					}
			...
			$row[] = ($accounts->created_by);			
					...
					else if($accounts->ref_salespaymentsdebit_id){
						$link = "";
						$title = '';
						$entry_of=8;
						$record_id=$accounts->ref_salespaymentsdebit_id;
					}
	...
	public function delete_transaction(){
		...
		if($entry_of==1){//transfer
			...
			echo $this->sales_model->delete_payment_from_account($id);
		}
		else if($entry_of==4){//sales return payments
			$this->load->model('sales_return_model');
			echo $this->sales_return_model->delete_payment_from_account($id);
		}
		else if($entry_of==5){//purchase payments
			$this->load->model('purchase_model');
			echo $this->purchase_model->delete_payment_from_account($id);
		}
		else if($entry_of==6){//purchase return payments
			$this->load->model('purchase_returns_model');
			echo $this->purchase_returns_model->delete_payment_from_account($id);
		}
		else if($entry_of==7){//Expenses
			$this->load->model('expense_model');
			echo $this->expense_model->delete_expenses_from_account($id);
		}
		else if($entry_of==8){//sales debit payments
			$this->load->model('sales_debit_model');
			echo $this->sales_debit_model->delete_payment_from_account($id);
		}

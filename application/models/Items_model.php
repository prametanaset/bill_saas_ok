...
class Items_model extends MY_Model {
	...
	{	
		...
		if($item_type=='Combo'){
			$this->db->where('a.item_group="Combo"');
		}
		...
		//$this->db->where("a.item_group!='Variants'");
			$this->db->where("(a.item_group is NULL || a.item_group = 'Single' || a.item_group = 'Combo')");
    ...
    /*
	* Check Circular Reference for Combo Items
	* Prevents adding a parent item as a child of itself (directly or indirectly)
	*/
	public function check_circular_reference($target_id, $current_id) {
		// 1. Immediate check: You can't add the item to itself
        if ($target_id == $current_id) {
            return true;
        }

		// 2. Check if the child (current_id) is a combo item that contains the parent (target_id)
		// Get all components of the child from db_combo_items
        $this->db->select('item_id');
        $this->db->where('parent_id', $current_id);
        $query = $this->db->get('db_combo_items');

        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
				// If the child contains the target, it's a cycle
                if ($row->item_id == $target_id) {
                    return true;
                }
				// Recursive step: traverse down
                if ($this->check_circular_reference($target_id, $row->item_id)) {
                    return true;
                }
            }
        }
	...
	public function save_record($modal_post=array()){
		...
		$hide_in_pos = isset($hide_in_pos) ? $hide_in_pos : 0;

		if($item_group=='Single' || $item_group=='Combo'){
			$initial = array();
			if ( $command == 'save' ) {
				$initial = array(
			    				'store_id' 					=> $store_id,
        ...
        // Insert Combo Items
        if ($item_group == 'Combo') {
            // Delete existing combo items if updating
            if ($command == 'update') {
                $this->db->where('parent_id', $item_id)->delete('db_combo_items');
            }

            $current_combo_row_count = $this->input->post('current_combo_row_count');
            $total_combo_cost = 0;

            for ($i = 1; $i <= $current_combo_row_count; $i++) {
                if (isset($_REQUEST['combo_item_id_' . $i]) && !empty($_REQUEST['combo_item_id_' . $i])) {
                    $component_id = $this->security->xss_clean(trim($_REQUEST['combo_item_id_' . $i]));
                    $quantity = $this->security->xss_clean(trim($_REQUEST['combo_item_qty_' . $i]));
                    $component_price = $this->security->xss_clean(trim($_REQUEST['combo_item_price_' . $i]));

                    // Circular Reference Check
                    if ($this->check_circular_reference($item_id, $component_id)) {
                        $this->db->trans_rollback();
                        return "Circular reference detected! Item cannot contain itself or its own parent.";
                    }

                    $total_combo_cost += ($component_price * $quantity);
                    ...
                    $this->db->insert('db_combo_items', $combo_data);
                }
            }
            
            // Update the calculated cost
            $this->db->set('purchase_price', $total_combo_cost)->where('id', $item_id)->update('db_items');
        }
	...
	public function get_details($id,$data){
		...
		//	$data['hsn']=$query->hsn;
			...
			$data['hide_in_pos']=$query->hide_in_pos;
    ...
    public function get_combo_items_in_row($parent_id) {
        if(empty($parent_id)){ return ''; }
        $q1 = $this->db->select('*')->from('db_combo_items')->where("parent_id", $parent_id)->get();
        $rowcount = 1;
        $result = '';
        foreach ($q1->result() as $res1) {
            $item_details = $this->db->select('*')->from('db_items')->where("id", $res1->item_id)->get()->row();
            ...
            $info = array(
                ...
                'total_cost' => $item_details->purchase_price * $res1->quantity
            );
            $result .= $this->return_combo_data_in_html_row($rowcount++, $info);
        }
        return $result;
    }

    public function return_combo_data_in_html_row($rowcount, $info) {
        ...
        return '<tr id="tr_combo_' . $rowcount . '">
            <td>' . $item_name . '<input type="hidden" name="combo_item_id_' . $rowcount . '" value="' . $item_id . '"></td>
            <td><input type="text" class="form-control text-right" name="combo_item_qty_' . $rowcount . '" id="combo_item_qty_' . $rowcount . '" value="' . $quantity . '" onkeyup="calculate_combo_cost(' . $rowcount . ')"></td>
            <td><input type="text" class="form-control text-right" name="combo_item_price_' . $rowcount . '" id="combo_item_price_' . $rowcount . '" value="' . $purchase_price . '" onkeyup="calculate_combo_cost(' . $rowcount . ')"></td>
            <td><input type="text" class="form-control text-right" name="combo_item_total_' . $rowcount . '" id="combo_item_total_' . $rowcount . '" value="' . $total_cost . '" readonly></td>
            <td><a class="fa fa-fw fa-trash-o text-red" style="cursor: pointer;font-size: 20px;" onclick="remove_combo_row(' . $rowcount . ')" title="Delete Item?"></a></td>
        </tr>';
    }
	...
	public function preview_labels(){
		...
		?>
		<div style=" height:6in !important;  width:1.6in !important; line-height: 16px !important; ">
			<div class="inner-div-2" style=" height:1.2in !important;  width:2.0in !important; line-height: 16px !important;">
				<div style="">
					...
					for($i=1;$i<=$rowcount;$i++){
						...
						if(isset($_POST['tr_item_id_'.$i]) && !empty($_POST['tr_item_id_'.$i])){
							...
							?>
							<div style="height:1.2in !important; line-height: 1.2in; width:2.0in !important; display: inline-block; <?=$page_break;?>  " class="label_border text-center">

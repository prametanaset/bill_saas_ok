...
class Reports_model extends MY_Model {
	...
	public function show_sales_debit_report(){
		extract($_POST);
		...
		$this->db->select("a.id,a.warehouse_id,a.debit_code,a.debit_date,b.customer_name,b.customer_code,a.grand_total,a.paid_amount,a.store_id");
	    
		if($customer_id!=''){
			$this->db->where("a.customer_id=$customer_id");
		}
		if($view_all=="no"){
			$this->db->where("(a.debit_date>='$from_date' and a.debit_date<='$to_date')");
		}
		$this->db->where("b.`id`= a.`customer_id`");
		$this->db->from("db_salesdebit as a");
		$this->db->from("db_customers as b");
		$this->db->select("CASE WHEN c.sales_code IS NULL THEN '' ELSE c.sales_code END AS sales_code");
		$this->db->join('db_sales as c','c.id=a.sales_id','left');
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				if($store_id==get_current_store_id()){
				echo "<td class='text-center'><a title='View Invoice' href='".base_url("sales_debit/invoice/$res1->id")."'>".$res1->debit_code."</a></td>";
				}
				else{
				echo "<td class='text-center'>".$res1->debit_code."</td>";	
				}

				
				echo "<td class='text-center'>".show_date($res1->debit_date)."</td>";
			...
			foreach ($q1->result() as $res1) {
				...
				if($res1->item_group=='Variants'){continue;}
					...
					/*if($available_qty_wh>0){*/
						...
						// COMBO LOGIC: Get components for display BEFORE using in Item Name
						$combo_info = "";
						$is_combo = ($res1->item_group == 'Combo');
						if($is_combo){
						    $components = $this->db->where('parent_id', $res1->item_id)->get('db_combo_items')->result();
						    if(!empty($components)){
						        $combo_info .= "<small class='text-muted'>";
						        foreach($components as $comp){
						            $comp_item = $this->db->select('item_name')->where('id', $comp->item_id)->get('db_items')->row();
                                    $c_name = $comp_item ? $comp_item->item_name : $comp->item_id;
						            $combo_info .= "<br>- " . $c_name . " (" . format_qty($comp->quantity) . ")";
						        }
						        $combo_info .= "</small>";
						    }
						}

						$str .= "<td>".$res1->item_name . $combo_info . "</td>";
						$str .= "<td class='text-center'>".$res1->brand_name."</td>";
						$str .= "<td>".$res1->category_name."</td>";
						
                        // Hide values for Combo items to prevent double counting
                        $disp_purchase_price = $is_combo ? '-' : store_number_format($res1->purchase_price);
                        $disp_price = $is_combo ? '-' : store_number_format($res1->price);
                        $disp_sales_price = $is_combo ? '-' : store_number_format($res1->sales_price);
                        $disp_value = $is_combo ? '-' : store_number_format($value);
                        $disp_stock_value = $is_combo ? '-' : store_number_format($stock_value_by_purchase_price);

						$str .= "<td class='text-right'>".$disp_purchase_price."</td>";
						$str .= "<td class='text-right'>".$res1->tax_name."[".$tax_type."]</td>";
						$str .= "<td class='text-right'>".$disp_price."</td>";
						$str .= "<td class='text-right'>".$disp_sales_price."</td>";
						$str .= "<td class='text-right'>".format_qty($available_qty_wh)."</td>";
						// REMOVED DUPLICATE COLUMN HERE
						$str .= "<td class='text-right'>".$disp_value."</td>";
						$str .= "<td class='text-right'>".$disp_stock_value."</td>";
						$str .= "</tr>";
						
                        // Do not add Combo value to Grand Total to avoid double counting (Components are already counted)
                        if(!$is_combo){
	...
	function _get_db_sales_debit_ids($store_id='',$from_date,$to_date){

		return $this->_create_query($store_id,'db_salesdebit','debit_date',$from_date,$to_date);
	...
	public function get_profit_loss_report(){
			...
			$sales_debit_ids = $this->_get_db_sales_debit_ids($store_id,$from_date,$to_date);
            ...
            //Closing Stock Price
            $this->db->select("SUM(stock * purchase_price) AS closing_stock_price");
            $this->db->select("SUM(stock * sales_price) AS closing_stock_price_sales");
            $this->db->from("db_items");
            if(!empty($store_id)){
                $this->db->where("store_id",$store_id);
            }
            $query_stock = $this->db->get()->row();
            $info['closing_stock_price'] = store_number_format($query_stock->closing_stock_price);
            $info['closing_stock_price_sales'] = store_number_format($query_stock->closing_stock_price_sales);
            ...
            //Use refined VAT column
            $purchase_tax_amt = $this->db->query("SELECT COALESCE(SUM(vat_amt),0) AS tax_amt FROM db_purchase WHERE id IN ($purchase_ids)")->row()->tax_amt;
			...
			$pur_total_amt = $pur_total + $purchase_discount_amt;

            $info['total_purchase_amt']=(store_number_format($pur_total_amt));
           ...
           // $pur_total-=$purchase_tax_amt;
            ...
            //Use refined VAT column
            $purchase_return_tax_amt = $this->db->query("SELECT COALESCE(SUM(vat_amt),0) AS tax_amt FROM db_purchasereturn WHERE id IN ($purchase_return_ids)")->row()->tax_amt;
         ...
         //   $info['sales_tax_amt']=(store_number_format($sales_vat));
			...
			//Use refined VAT column
            $tax_in_dis = $this->db->query("SELECT COALESCE(SUM(vat_amt),0) AS tax_amt FROM db_sales WHERE id IN ($sales_ids) AND sales_status='Final'")->row()->tax_amt;
		
			 $info['sales_tax_amt']=(store_number_format($tax_in_dis ));
     ...
     //  $sales_return_discount_amt=$query->discount_amt;
         ...
         //   $info['sales_return_tax_amt']=(store_number_format($sales_return_tax_amt));
          ...
          //  $info['return_coupon_discount_amt']=(store_number_format($coupon_discount_amt));
            ...
            $sal_return_total=$query->sal_total;
          //  $sal_return_total-=$sales_return_tax_amt;
            $info['sale_return_total']=(store_number_format($sal_return_total - $sal_return_other_charges_amt + $sales_return_discount_amt));
            
            
            /***********Sales Debit**********/
            $this->db->select("COALESCE(SUM(tax_amt),0) AS tax_amt");
            $this->db->select("COALESCE(SUM(discount_amt),0) AS discount_amt");
            $this->db->from("db_salesitemsdebit");
            $this->db->where("debit_id in (".$sales_debit_ids.")");
            $query = $this->db->get()->row();

            //total sales debit tax amt
            $sales_debit_tax_amt=$query->tax_amt;
            
            $this->db->select("COALESCE(SUM(grand_total),0) AS sal_total");
            $this->db->select("COALESCE(SUM(other_charges_amt),0) AS other_charges_amt");
            $this->db->select("COALESCE(SUM(tot_discount_to_all_amt),0) AS tot_discount_to_all_amt");
            $this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount");
            $this->db->where("id in (".$sales_debit_ids.")");
            $this->db->from("db_salesdebit");
            $query = $this->db->get()->row();
            

            //total sales debit amt
            $sal_debit_total=$query->sal_total;
            $info['sal_debit_total']=(store_number_format($sal_debit_total));

            //Other Charge of Sales debit entry
            $sal_debit_other_charges_amt=$query->other_charges_amt;
            $info['sal_debit_other_charges_amt']=(store_number_format($sal_debit_other_charges_amt));
            
            //Discount 
            $sales_debit_discount_amt =$query->tot_discount_to_all_amt;
            $info['sales_debit_discount_amt']=(store_number_format($sales_debit_discount_amt));

            //sales Debit Paid Amount;
            $sales_debit_paid_amount=$query->paid_amount;
            $info['sales_debit_paid_amount']=(store_number_format($sales_debit_paid_amount));

             //Due
             $sales_debit_due_total=$sal_debit_total - $query->paid_amount;
             $info['sales_debit_due_total']=(store_number_format($sales_debit_due_total));

             //total sales debit amt
             $info['sale_debit_total']=(store_number_format($sal_debit_total - $sal_debit_other_charges_amt + $sales_debit_discount_amt));

            //Use refined VAT column
            $tax_debit = $this->db->query("SELECT COALESCE(SUM(vat_amt),0) AS tax_amt FROM db_salesdebit WHERE id IN ($sales_debit_ids)")->row()->tax_amt;
            $info['sales_debit_tax_amt']=(store_number_format($tax_debit));
          ...
          //  $net_sales = $sal_cost;//ACTUAL SALES VALUE
		//	$net_sales = ($sal_cost - $sal_item_pur_price) - $sales_discount_all;
		   $net_sales = ($sal_total - $sal_item_pur_price) ;  // grand total - ราคาทุน(ไม่รวมvat)
            ...
            $net_return = ($ret_cost + $sal_return_other_charges_amt) - $sales_return_discount_amt ;//ACTUAL RETURN VALUE

			
			//total sales return tax amt

			//Use refined VAT column
            $tax_return = $this->db->query("SELECT COALESCE(SUM(vat_amt),0) AS tax_amt FROM db_salesreturn WHERE id IN ($sales_return_ids)")->row()->tax_amt;

            $info['sales_return_tax_amt']=(store_number_format($tax_return));

            //TO FIND GROSS PROFIT = (SALES PRICE - PURCHASE PRICE OF ITEM) - (SALES RETURN - PURCHASE PRICE OF ITEM)
            
            //  Gross profit = Sales (Excl. Tax) - Purchase price (Excl. Tax) - $tax_in_dis
            $gross_profit =  $net_sales - ($net_return - $ret_item_pur_price);
            // Subtract net tax from gross profit to get pre-tax gross profit
           // $real_gross_profit = $gross_profit - ($tax_in_dis - $tax_return) -  $net_return;
			 $real_gross_profit = $gross_profit;                // $net_return  + $sal_return_other_charges_amt

            //Sales Debit Profit
            $debit_details = $this->get_sales_debit_item_sum($sales_debit_ids);
            $debit_item_pur_price = $debit_details['purchase_price'];
            $debit_cost = $debit_details['debit_price'];
            $net_debit = ($debit_cost + $sal_debit_other_charges_amt) - $sales_debit_discount_amt ;
            $debit_profit = ($net_debit - $debit_item_pur_price);

            $sales_profit = $net_sales;
            $return_profit_loss = ($net_return - $ret_item_pur_price);
            $final_gross_profit = $sales_profit + $debit_profit - $return_profit_loss;

            $info['sales_profit']=(store_number_format($sales_profit));
            $info['debit_profit']=(store_number_format($debit_profit));
            $info['return_profit_loss']=(store_number_format($return_profit_loss));

            $info['net_sales']=(store_number_format($sal_cost));
            $info['sales_return_total']=(store_number_format($ret_cost - $tax_return));
            $info['gross_profit']=(store_number_format( $final_gross_profit));  // ยอดขายสุทธิ - (ยอดขายคืน - ราคาทุน )

            
            //Tax
            //Sales & Retuern tax, because we are calculating sum
            $tot_tax = $sal_tax_amt - $ret_tax_amt + $sales_debit_tax_amt;
            //Now you got valid tax

            //To find net profit now need to deduct valid tax from gross profit
            //also deduct expenses
            // Net Profit = Pre-tax Gross Profit - Expenses
            $net_profit = $final_gross_profit - $exp_total ;
            
            $info['tot_net_profit']=(store_number_format($net_profit));

			$tot_payment = ( $sales_paid_amount + $sales_debit_paid_amount - $sales_return_paid_amount ) - ( $purchase_paid_amount - $purchase_return_paid_amount );

			 $info['tot_payment']=(store_number_format($tot_payment));
	...
	public function get_sales_debit_item_sum($sales_debit_ids){
		$this->db->select("a.debit_qty,a.purchase_price,a.total_cost,a.tax_amt,a.tax_type");
		$this->db->select("b.tax");
		
		$this->db->from('db_salesitemsdebit a');
		$this->db->where("a.debit_id in (".$sales_debit_ids.")");
		...
		if($Q1->num_rows()>0){
			foreach ($Q1->result() as $res1){
				$debit_qty = $res1->debit_qty;

				$purchase_price = ($res1->tax_type == 'Exclusive') ? $res1->purchase_price : original_cost($res1->purchase_price,$res1->tax,'Inclusive');


				$debit_price = $res1->total_cost;
				$tax_amt = $res1->tax_amt;

				$tot_pur_price = $debit_qty * $purchase_price;

				$sum_pur_price+=$tot_pur_price;
				$sum_debit_price+=$debit_price;
				$sum_tax_amt+=$tax_amt;
			}
		}
...
//		$this->db->group_by("c.`item_id`");
	...
	public function show_sales_tax_report(){
		...
		$this->db->select("a.warehouse_id,a.store_id,a.vat,a.subtotal,a.other_charges_amt");
		...
		$this->db->select("a.round_off, a.vat_amt, a.taxable_amount");
		...
		if($q1->num_rows()>0){
			...
			$tot_grand_total=0;
			foreach ($q1->result() as $res1) {
				$tot_discount_to_all_amt = $res1->tot_discount_to_all_amt;
				$other_charges_amt = $res1->other_charges_amt;
				$grand_total = $res1->grand_total;
				$subtotal = $res1->subtotal;
				
				// VAT rate from sales or store fallback
				$vat_no = floatval($res1->vat);
				if($vat_no == 0){
					$vat_no = floatval($resVat->vat_no); // Default to store VAT if not saved in sale
				}

				/* Find Tax Amount from items */
				$q2 = $this->db->select("COALESCE(sum(tax_amt),0) as tax_amt")
								->where("sales_id",$res1->id)->get("db_salesitems")->row();
				$item_tax_amt = $q2->tax_amt;

				// Calculation logic similar to print-sales-invoice-4.php
				// Total taxable = Grand Total before VAT
				// Since grand_total = taxable + vat_amt
				// taxable = grand_total / (1 + (vat_no/100))
				
				$total_vatable = $res1->taxable_amount;
				$vat_amt = $res1->vat_amt;

				// Ensure we don't have negative values
				$total_vatable = ($total_vatable > 0) ? $total_vatable : 0;
				$vat_amt = ($vat_amt > 0) ? $vat_amt : 0;
				...
				echo "<td>".$res1->customer_name."</td>";
				echo "<td class='text-right'>".store_number_format($tot_discount_to_all_amt)."</td>";
				echo "<td class='text-center'>".$vat_no."%</td>";			
				echo "<td class='text-right'>".store_number_format($vat_amt)."</td>";		
				echo "<td class='text-right'>".store_number_format($res1->grand_total)."</td>";
				echo "</tr>";
			 ...
			 */
			 ...
			 <td class='text-right text-bold' ></td>
					 <td class='text-right text-bold' ></td>
					 <td class='text-right text-bold' ></td>			
	  			     <td class='text-right text-bold' ><b>รวม :</b></td
	...
	public function show_purchase_tax_report(){
		...
		$this->db->select("a.tot_discount_to_all_amt");
		$this->db->select("a.round_off, a.vat_amt, a.taxable_amount, a.vat");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$q2 = $this->db->select("COALESCE(sum(tax_amt),0) as tax_amt")
								->select("COALESCE(sum(price_per_unit),0) as price_per_unit")
								->select("COALESCE(sum(discount_amt),0) as tot_discount_to_all_amt")
								->where("purchase_id",$res1->id)->get("db_purchaseitems")->row();
				$tax_amt = $q2->tax_amt;
			
				$price_per_unit = $q2->price_per_unit;
				$discount_all_amt = $res1->tot_discount_to_all_amt;

				$vat_no = floatval($res1->vat);
				$total_vatable = $res1->taxable_amount;
				$vat_amount =  $res1->vat_amt;
				...
				echo "<td>".$res1->supplier_name."</td>";
				echo "<td class='text-right'>".store_number_format($discount_all_amt)."</td>";
				echo "<td class='text-center'>" .store_number_format($vat_no). "%</td>";
		
				echo "<td class='text-right'>".store_number_format($vat_amount)."</td>";
			
				echo "<td class='text-right'>".store_number_format($res1->grand_total)."</td>";
				echo "</tr>";
				$tot_price_per_unit+=$price_per_unit;
				$tot_discount_amt+=$discount_all_amt;
				$tot_tax_amt+=$vat_amount;			
				$tot_grand_total+=$res1->grand_total;

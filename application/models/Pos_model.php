...
class Pos_model extends MY_Model {
	      ...
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
	        	...
	        	if($w_stock <1 && !$service_bit){
	        		...
	        		$bg_color="background-color:#CCCCFF";
	        	}
	          ...
	          <div class="box box-default item_box" id="div_'.$res2->id.'" onclick="'.$str.'"
	            ...
	            <div class="box-body box-profile">
	              ...
	              <lable class="text-center search_item" style="font-weight: bold;font-family: sans-serif;" id="item_'.$i.'">'.substr($res2->item_name,0,70).'</label><br>
	              <span class="" style="font-family: sans-serif;font-size:100%; " >'.$CI->currency(store_number_format($item_sales_price)).'
	              </span>
	            </div>
	...
	public function pos_save_update(){//Save or update sales
		...
		$store_id=(store_module() && is_admin() && !empty($store_id)) ? $store_id : get_current_store_id();
		$rowcount 			=$hidden_rowcount;
		$sales_date 		=date("Y-m-d",strtotime($CUR_DATE));
		//$points 			= (empty($points_use)) ? 'NULL' : $points_use;
		$discount_input 	= (empty($discount_input)) ? 0 : str_replace(',', '', $discount_input);
		$tot_disc 		    = (empty($tot_disc)) ? 0 : str_replace(',', '', $tot_disc);
		$tot_grand 		    = (empty($tot_grand)) ? 0 : str_replace(',', '', $tot_grand);
        
        // Backup global discount variables to prevent overwrite in item loop
        $global_discount_input = $discount_input;
        $global_discount_type = (isset($discount_type)) ? $discount_type : 'in_percentage';
		...
		$sales_entry_init = array(
				    				...
				    				'coupon_amt' 				=> (isset($coupon_discount_amt) ? $coupon_discount_amt : 0),
		...
		if($command=='update'){
				$sales_entry = array(
		    				...
		    				/*Discount*/
		    				'discount_to_all_input' 	=> $global_discount_input,
		    				'discount_to_all_type' 		=> $global_discount_type,
				...
				$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
											->where("ref_salespayments_id in (select id from db_salespayments where sales_id in ($sales_id))")
											->group_by("debit_account_id,credit_account_id")
											->get("ac_transactions");
				//ACCOUNT RESET END

				//DELETE FROM ACCODE TRANSACTIONS
				$this->db->where("ref_salespayments_id in (select id from db_salespayments where sales_id in ($sales_id))")->delete("ac_transactions");
		...
		for($i=0;$i<$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && trim($_REQUEST['tr_item_id_'.$i])!=''){
				...
				if(floatval($tax_value) > $max_tax_rate) { $max_tax_rate = floatval($tax_value); }
				if(floatval($tax_value) == 0){ $tot_price_zero_vat += floatval($total_cost); }
                ...
                $discount_input = str_replace(',', '', $discount_input);
        ...
        // Use the global discount directly sent from POS
        $discount_total = round(floatval($global_discount_input), 2);
        if($global_discount_type == 'in_percentage'){
           // Calculate discount from GROSS Inclusive subtotal (Items + Other Charges)
           $discount_total = round(($cal_subtotal + $cal_other_charges_amt) * $global_discount_input / 100, 2);
        }
	...
	public function update_items_quantity($item_id){
        ...
        /* COMBO LOGIC: Deduct Stock if Component of Sold Combos */
        $combo_sales_deduction = 0;
        // Find which combos this item belongs to
        $parent_combos = $this->db->select('parent_id, quantity')->where('item_id', $item_id)->get('db_combo_items');
        
        if($parent_combos->num_rows() > 0){
            foreach($parent_combos->result() as $parent){
                // Sales of the Parent Combo
                $parent_sales = $this->db->query("select coalesce(SUM(sales_qty),0) as sales_qty from db_salesitems where item_id='".$parent->parent_id."' and sales_status='Final'")->row()->sales_qty;
                
                // Returns of the Parent Combo
                $parent_returns = $this->db->query("select coalesce(SUM(return_qty),0) as return_qty from db_salesitemsreturn where item_id='".$parent->parent_id."'")->row()->return_qty;
                
                $net_parent_sold = $parent_sales - $parent_returns;
                // Deduct (Parent Sold * Qty of Component in Combo)
                $combo_sales_deduction += ($net_parent_sold * $parent->quantity);
            }
        }

		$stock=((($stock_qty+$pu_tot_qty)-$sl_tot_qty)+$sl_return_tot_qty)-$pu_return_tot_qty - $combo_sales_deduction;
	...
	public function edit_pos($sales_id){
	    ...
	    if($q2->num_rows()>0){
		  ...
		  if($rows>0){
		  	...
		  	echo "<<<###>>>".$discount_input."<<<###>>>".$discount_type."<<<###>>>".$customer_id."<<<###>>>".$store_id."<<<###>>>".$warehouse_id."<<<###>>>".$sales_note."<<<###>>>".$invoice_terms."<<<###>>>".$reference_no."<<<###>>>".$sales_date;
	...
	public function hold_list_save_update(){//Save or update sales
		...
		$sales_entry = array();
        log_message('error', 'POS Debug: POST grand_total='.$this->input->post('grand_total'));
        //Import post data from form
		for($i=0;$i<$rowcount;$i++){

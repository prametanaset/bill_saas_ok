...
class Store_profile_model extends MY_Model {
	...
	public function update_store(){
		...
		/*end*/
			...
			// Preserve current status to prevent accidental reset to 0
			$current_store_rec = $this->db->select('status')->where('id', $q_id)->get('bill_xml.db_store')->row();
			if($current_store_rec){
				$data['status'] = $current_store_rec->status;
			}

			$store_code_count=$this->db->query("select count(*) as store_code_count from bill_xml.db_store where upper(store_code)=upper('$store_code') and id<>$q_id")->row()->store_code_count;
			if($store_code_count>0){
				echo "ขออภัย! รหัสร้านค้านี้มีอยู่ในระบบเรียบร้อยแล้ว!\nกรุณาเปลี่ยนรหัสร้านค้า";exit();
			}

            // Update master database
			$q1 = $this->db->where('id',$q_id)->update('bill_xml.db_store', $data);
            // Update tenant database as well just in case they are meant to be synced
            $q2 = $this->db->where('id',$q_id)->update('db_store', $data);
			if($q1){
	...
	public function get_details($id){
		...
		$query1=$this->db->query("select * from bill_xml.db_store where upper(id)=upper('$id')");

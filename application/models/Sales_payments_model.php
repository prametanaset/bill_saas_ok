...
class Sales_payments_model extends MY_Model {
	...
	private function _get_datatables_query()
	{
	    ...
	    if(!empty($payment_from_date)) {
	    	$payment_from_date = system_fromatted_date($payment_from_date);
	    	if($payment_from_date!='1970-01-01'){
	    		$this->db->where("a.payment_date>=",$payment_from_date);
	    	}
	    }

	    $payment_to_date = $this->input->post('payment_to_date');
	    if(!empty($payment_to_date)) {
	    	$payment_to_date = system_fromatted_date($payment_to_date);
	    	if($payment_to_date!='1970-01-01'){
	    		$this->db->where("a.payment_date<=",$payment_to_date);
	    	}
	    }

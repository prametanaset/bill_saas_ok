...
class Sales_model extends MY_Model {
	...
	public function __construct()
	{
		...
		// Create db_receipts table if not exists (Required because Sales_model queries it)
		...
		// Dashboard sales datatable queries invoice codes via subquery, so the table must exist for all tenants.
		...
		// Self-heal older tenants where db_invoice exists but misses newer columns/settings.
		if (!$this->db->field_exists('reference_no', 'db_invoice')) {
			$this->db->query("ALTER TABLE `db_invoice` ADD `reference_no` varchar(100) DEFAULT NULL AFTER `invoice_date`");
		}
		...
		// Auto-add taxable_amount and vat_amt columns if not exist
		// Note: db_sales usually exists, we just ensure these new columns are there
		if (!$this->db->field_exists('taxable_amount', 'db_sales')) {
			$this->db->query("ALTER TABLE `db_sales` ADD `taxable_amount` double(14,2) DEFAULT '0.00' AFTER `subtotal` ");
		}
		if (!$this->db->field_exists('vat_amt', 'db_sales')) {
			$this->db->query("ALTER TABLE `db_sales` ADD `vat_amt` double(14,2) DEFAULT '0.00' AFTER `taxable_amount` ");
		}
	}
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("a.*, b.customer_name, c.code");
		$this->db->select("(SELECT COUNT(*) FROM db_sales WHERE reference_no = a.sales_code AND id != a.id AND store_id = a.store_id) AS referenced_count");
		$this->db->select("(SELECT GROUP_CONCAT(receipt_code SEPARATOR ', ') FROM db_receipts WHERE sales_id = a.id AND status = 1) AS receipt_codes");
		$this->db->select("(SELECT GROUP_CONCAT(invoice_code SEPARATOR ', ') FROM db_invoice WHERE sales_id = a.id AND status = 1) AS invoice_codes");
	...
	function get_datatables()
	{
		...
		if ($this->db->field_exists('reference_type', 'db_sales')) {
			$this->db->select('a.reference_type');
		}	
		// ตรวจว่ามีใบเพิ่มหนี้ (debit note) หรือไม่
		$this->db->select('(SELECT COUNT(*) FROM db_salesdebit WHERE db_salesdebit.sales_id = a.id) as debit_bit');
		if($_POST['length'] != -1)
	...
	public function verify_save_and_update(){
	    ...
	    if($other_charges_type==''){$other_charges_type=null;}
	    ...
	    if(empty(trim($count_id))) {
	    	echo "จำเป็นต้องกรอกเลขที่ (Invoice Number) เสมอ!";exit;
	    }
	    else{
	    	if(!is_numeric($count_id)){
	    		echo "(Invoice Number) ต้องเป็นตัวเลขเท่านั้น!";exit;		
	    	}
	    }
	    ...
	    //Verify Sales Code
		$this->db->where("sales_code",$init_code.str_pad($count_id, 4, '0', STR_PAD_LEFT));
		if($command=='update'){
	    ...
	    $sales_entry_init = array(
							'init_code' 				=> $init_code,
		    				'count_id' 					=> $count_id,
		    				'sales_code' 				=> $init_code.str_pad($count_id, 4, '0', STR_PAD_LEFT),//get_init_code('sales'),
		    				/*Coupon disocunt amt*/
		...
		$tot_price_zero_vat = 0;
		$tot_price_vatable_inc = 0;
		$max_tax_rate = 0;
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				if(floatval($unit_tax) > $max_tax_rate) { $max_tax_rate = floatval($unit_tax); }
				if(floatval($unit_tax) == 0){ $tot_price_zero_vat += floatval($total_cost); }
				else{ $tot_price_vatable_inc += floatval($total_cost); }
                ...
                // Final Calculation Logic (Verified Sequence A)
        $res1=$this->db->query("select * from db_store where id=$store_id")->row();

        // Step 1: Get subtotal from saved DB items (sum of all item total_cost)
        $this->db->select_sum('total_cost');
        $this->db->where('sales_id', $sales_id);
        $query_sum = $this->db->get('db_salesitems');
        $cal_subtotal = floatval($query_sum->row()->total_cost);

        // Step 2: Calculate Other Charges amount (Percentage or Fixed)
        $cal_other_charges_amt = 0;
        if(!empty($other_charges_input)){
             if($other_charges_type=='Percentage'){
                 $cal_other_charges_amt = ($cal_subtotal * floatval($other_charges_input)) / 100;
             } else {
                 $cal_other_charges_amt = floatval($other_charges_input);
             }
        }
        
        // Determine prevailing VAT rate for de-taxing charges (Priority: Selected > Store > Max Item)
        $vat_no = 0; 
        if(!empty($other_charges_tax_id)){
             $q_tax_ptr = $this->db->query("select tax from db_tax where id=$other_charges_tax_id")->row();
             if($q_tax_ptr){ $vat_no = floatval($q_tax_ptr->tax); }
        }
        if($vat_no == 0 && isset($res1->vat_no)) { $vat_no = floatval($res1->vat_no); }
        if($vat_no == 0 && $max_tax_rate > 0) { $vat_no = $max_tax_rate; }

        $total_in_tax = $tot_price_vatable_inc;
        $item_vat_inc = ($vat_no > 0) ? (($total_in_tax * 100) / (100 + $vat_no)) : $total_in_tax;
        $service_fee = ($vat_no > 0) ? (($cal_other_charges_amt * 100) / (100 + $vat_no)) : $cal_other_charges_amt;
        $item_exempt = $tot_price_zero_vat;
        
        // Use the global discount directly sent from Sales
        $discount_total = round(floatval($discount_to_all_input), 2);
        if($discount_to_all_type == 'in_percentage'){
           // Calculate discount from GROSS Inclusive subtotal (Items + Other Charges)
           $discount_total = round(($cal_subtotal + $cal_other_charges_amt) * $discount_to_all_input / 100, 2);
        }

        // Extract VAT from the discount before proportionally spreading it (Sync with Frontend)
        $discount_base = ($discount_total * 100) / (100 + $vat_no);

        $subtotal_before_discount = round($item_vat_inc + $item_exempt + $service_fee, 10);
        $discount_ratio = ($subtotal_before_discount != 0) ? ($discount_base / $subtotal_before_discount) : 0;

        $taxable_amount = round(($item_vat_inc + $service_fee) * (1 - $discount_ratio), 2);
        $vat_amt = ($vat_no > 0) ? round(($taxable_amount / 100) * $vat_no, 2) : 0;
        $net_exempt_group = round($item_exempt * (1 - $discount_ratio), 2);

        $final_grand_total = round(($taxable_amount + $vat_amt) + $net_exempt_group, 2);

        // Update db_sales with calculated total
        $this->db->where('id', $sales_id);
        $this->db->update('db_sales', array(
            'grand_total' => $final_grand_total,
            'subtotal'    => $cal_subtotal,
            'other_charges_amt' => $cal_other_charges_amt,
            'tot_discount_to_all_amt' => $discount_total,
            'vat_amt'     => $vat_amt,
            'taxable_amount' => $taxable_amount,
            'vat'         => $vat_no,
            'total_amount' => round($final_grand_total - $vat_amt, 2)
        ));
		...
		if($amount>0 && !empty($payment_type)){
			...
			*/
			...
			// 		echo "จำนวนเงินที่ต้องจ่ายไม่ควรเกินจำนวนเงินในใบแจ้งหนี้!!\nกรุณาตรวจสอบการชำระเงินครั้งก่อนๆ ด้วย.";exit;
			// 	}
			// }
	...
	function update_sales_payment_status_by_sales_id($sales_id,$customer_id){
		...
		$payment_status='';
		if($sum_of_payments>0 && $sum_of_payments>=$payble_total){
			$payment_status="Paid";
		}
	...
	public function delete_sales($ids){
      	...
      	if($q12->num_rows()>0){
      		...
      		echo "<br>คืนสินค้าแล้ว, โปรดลบการคืนสินค้าก่อนที่จะลบ Invoice ฉบับจริง";
      		exit;
      	}

      	//ACCOUNT RESET
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_salespayments_id in (select id from db_salespayments where sales_id in ($ids))")
		...
		$this->db->where("ref_salespayments_id in (select id from db_salespayments where sales_id in ($ids))")->delete("ac_transactions");

		$this->db->where("sales_id in ($ids)");
		$this->db->delete("db_salespayments");
		#----------------------------------
	...
	public function return_quotation_list($quotation_id){
		...
		foreach ($q1->result() as $res1) {
			...
			$info = array(
							...
							'item_name' 				=> ($res2) ? $res2->item_name : 'Item Not Found',
							'item_available_qty' 		=> ($res2) ? $res2->stock : 0,
							'item_price' 				=> ($res2) ? $res2->price : 0, 
							'item_sales_price' 			=> $res1->price_per_unit, 
							'item_tax_name' 			=> ($q3) ? $q3->tax_name : 'Tax Not Found', 
							'item_sales_qty' 			=> $res1->quotation_qty, 
							'item_tax_id' 				=> $res1->tax_id, 
							'item_tax' 					=> ($q3) ? $q3->tax : 0, 
							'item_tax_type' 			=> $res1->tax_type, 
	...
	public function return_sales_list($sales_id){
		extract($_POST);
		$is_sell_again = (isset($is_sell_again)) ? $is_sell_again : 0;
		$q_sales=$this->db->select("warehouse_id")->from("db_sales")->where("id=$sales_id")->get()->row();
		$warehouse_id = ($q_sales) ? $q_sales->warehouse_id : null;
		...
		foreach ($q1->result() as $res1) {
			...
			$actual_stock = ($res2) ? total_available_qty_items_of_warehouse($warehouse_id, null, $res1->item_id) : 0;
			$item_available_qty = ($res2) ? ($is_sell_again ? $actual_stock : ($actual_stock + $res1->sales_qty)) : $res1->sales_qty;
	...
	public function delete_payment_from_account($payment_id){
		...
		//UNLINK FROM ACCOUNT Instead of Delete
		$q1=$this->db->query("update db_salespayments set account_id=NULL where id='$payment_id'");
		
		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_salespayments_id in ($payment_id)")->delete("ac_transactions");

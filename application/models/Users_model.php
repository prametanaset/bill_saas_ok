...
class Users_model extends MY_Model {
	...
	public function status_update($id,$status){
        ...
        //Restrict deactivating Store Admin
        $user = $this->db->select('store_id')->where('id', $id)->get('db_users')->row();
        if($user){
            $store = $this->db->select('user_id')->where('id', $user->store_id)->get('bill_xml.db_store')->row();
            if($store && $store->user_id == $id && !is_admin()){
                echo "Restricted! คุณไม่สามารถปิดการใช้งาน Store Admin ได้!";exit();
            }
        }
	...
	public function delete_user($id){
        ...
        //Restrict deleting Store Admin
        $user = $this->db->select('store_id')->where('id', $id)->get('db_users')->row();
        if($user){
            $store = $this->db->select('user_id')->where('id', $user->store_id)->get('bill_xml.db_store')->row();
            if($store && $store->user_id == $id){
                echo "Restricted! คุณไม่สามารถลบ Store Admin ได้! หากต้องการลบ กรุณาลบที่เมนู Store List";exit();
            }
        }

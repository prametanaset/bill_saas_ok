...
class Quotation_model extends MY_Model {
	...
	public function verify_save_and_update(){
		...
		$other_charges_input = isset($other_charges_input) ? $other_charges_input : 0;
		$other_charges_tax_id = isset($other_charges_tax_id) ? $other_charges_tax_id : 0;
		$other_charges_type = isset($other_charges_type) ? $other_charges_type : 'Fixed';
		$discount_to_all_input = isset($discount_to_all_input) ? $discount_to_all_input : 0;
		$discount_to_all_type = isset($discount_to_all_type) ? $discount_to_all_type : 'Fixed';
		...
		// Calculate VAT and Taxable Amount (Sequence A Prorating Logic)
		...
		$discount_total = floatval($tot_discount_to_all_amt);
        
        // Extract VAT from the discount before proportionally spreading it (Sync with POS)
		...
		$final_grand_total = round($total_net_taxable + $total_vat_amount + $exempt_amount, 2);

		$this->db->where('id', $quotation_id);
		$this->db->update('db_quotation', array(
	...
	public function delete_quotation($ids){
		...
		$this->db->where("quotation_id in ($ids)");
		$this->db->delete("db_quotationitems");
		#----------------------------------

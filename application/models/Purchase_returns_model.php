...
class Purchase_returns_model extends MY_Model {
	//Datatable start
	...
	public function verify_save_and_update(){
        ...
        $res1=$this->db->query("select * from db_store where id=" . get_current_store_id())->row();
        ...
        $query_sum = $this->db->get('db_purchaseitemsreturn');
        $cal_subtotal = floatval($query_sum->row()->total_cost);

        // Step 2: Calculate Other Charges amount (Percentage or Fixed) - Purchase return usually doesn't have other charges input in UI, but logic here for safety
        $cal_other_charges_amt = floatval($other_charges_amt);
        ...
        // Fetch items grouping for precision
        ...
        foreach($items_q->result() as $row){
            ...
            if($tax_rate > 0){
                $tot_price_vatable_inc += floatval($row->total_cost);
            } else {
                $tot_price_zero_vat += floatval($row->total_cost);
            }
        }
        
        if($vat_no == 0 && $max_tax_rate > 0) { $vat_no = $max_tax_rate; }

        $item_vat_inc = ($vat_no > 0) ? (($tot_price_vatable_inc * 100) / (100 + $vat_no)) : $tot_price_vatable_inc;
		...
		$this->db->where('id', $return_id)->update('db_purchasereturn', array(
	...
	public function delete_payment_from_account($payment_id){
		...
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									...
									->get("ac_transactions");
		//ACCOUNT RESET END
		$purchasereturn = $this->db->query("select return_id,supplier_id from db_purchasepaymentsreturn where id=$payment_id")->row();
		$return_id = $purchasereturn->return_id;
		$supplier_id = $purchasereturn->supplier_id;

		//UNLINK FROM ACCOUNT
		$q1=$this->db->query("update db_purchasepaymentsreturn set account_id=NULL where id='$payment_id'");

		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_purchasepaymentsreturn_id in ($payment_id)")->delete("ac_transactions");
	...
	public function delete_return($ids){
      	...
      	//ACCOUNT RESET
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_purchasepaymentsreturn_id in (select id from db_purchasepaymentsreturn where return_id in ($ids))")
									->group_by("debit_account_id,credit_account_id")
									->get("ac_transactions");
		//ACCOUNT RESET END
		...
		if($q1->num_rows()>0){
			...
			$this->db->delete("db_purchaseitemsreturn");

			//DELETE FROM ACCODE TRANSACTIONS
			$this->db->where("ref_purchasepaymentsreturn_id in (select id from db_purchasepaymentsreturn where return_id in ($ids))")->delete("ac_transactions");

			$this->db->where("return_id in ($ids)");
			$this->db->delete("db_purchasepaymentsreturn");

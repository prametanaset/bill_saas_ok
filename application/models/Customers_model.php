...
class Customers_model extends MY_Model {
	...
	public function verify_and_save(){
		...
		if($query2->num_rows()>0 && !empty($mobile)){
			return "ขออภัย หมายเลขโทรศัพท์นี้มีอยู่แล้ว.";;
		}
	...
	public function update_customers(){
		...
		if($q_id==1){
			echo "ขออภัย! ข้อมูลนี้ถูกจำกัด! ไม่สามารถอัปเดตได้";exit();
		}
	...
	public function update_status($id,$status){
		...
		if($id==1){
			echo "บันทึกนี้ถูกจำกัดสิทธิ์! ไม่สามารถอัปเดตสถานะได้";exit();
		}
	...
	public function delete_customers_from_table($ids){
		...
		if($ids==1){
			echo "บันทึกนี้ถูกจำกัดสิทธิ์! ไม่สามารถลบสถานะได้";exit();
		}
		$q1 = $this->db->query("select count(*) as tot_entrys from db_sales where customer_id in ($ids)");
		if($q1->row()->tot_entrys >0 ){
			echo "ตรวจพบ(Sales Invoices) ของลูกค้ารายนี้อยู่ในระบบ! กรุณาลบ Sales Invoices!";exit();
		}
		  ...
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        ...
		        <h4 class="modal-title text-center">จ่ายเงินค้างชำระ</h4>
		      </div>
		    ...
		    <div class="row">
		      	...
		      	<div class="row invoice-info">
			        <div class="col-sm-12 invoice-col">
			          ...
			          <address>
			            ...
			            <?php echo (!empty(trim($customer_tax_number))) ? $this->lang->line('branch').": ".$customer_tax_number."<br>" : '';?>
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" onclick="save_payment(<?=$customer_id;?>)" class="btn bg-primary btn-lg place_order btn-lg payment_save">บันทึก<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
		  ...
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		          <span aria-hidden="true">&times;</span></button>
		        <h4 class="modal-title text-center">จ่ายเงินค้างชำระที่คืนสินค้า </h4>
		      </div>
		      <div class="modal-body">
		    ...
		    <div class="row">
		      ...
		      <div class="modal-footer">
		        <button type="button" class="btn btn-default btn-lg" data-dismiss="modal">ปิด</button>
		        <button type="button" onclick="save_return_due_payment(<?=$customer_id;?>)" class="btn bg-primary btn-lg place_order btn-lg return_due_payment_save">บันทึก<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>

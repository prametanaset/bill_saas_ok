...
class Gateways_model extends MY_Model {
	...
	public function update_gateways(){
		...
		//Check if stripe entry exists, if not insert default
		$q1 = $this->db->query("select * from db_stripe where id=1");
		if($q1->num_rows()==0){
			$this->db->insert('db_stripe', ['store_id'=>1]);
		}

		$query1 = $this->db->where('id',1)->update('db_stripe', $stripe);
		if (!$query1){
			return "failed";
		}

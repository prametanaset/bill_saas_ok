...
class Sales_return_model extends MY_Model {
	...
	private function _get_datatables_query()
	{
	    ...
	    $return_from_date = $this->input->post('return_from_date');
	    if(!empty($return_from_date)) {
	    	$return_from_date = system_fromatted_date($return_from_date);
	    	if($return_from_date!='1970-01-01'){
	    		$this->db->where("a.return_date>=",$return_from_date);
	    	}
	    }

	    $return_to_date = $this->input->post('return_to_date');
	    if(!empty($return_to_date)) {
	    	$return_to_date = system_fromatted_date($return_to_date);
	    	if($return_to_date!='1970-01-01'){
	    		$this->db->where("a.return_date<=",$return_to_date);
	    	}
	    }
	...
	function get_datatables()
	{
		...
		$this->db->select('c.grand_total as sales_grand_total'); // มูลค่าตามใบกำกับภาษีเดิม
		if($_POST['length'] != -1)
	...
	public function verify_save_and_update(){
		...
		$this->db->update('db_salesreturn', array(
	...
	public function delete_return($ids){
		...
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_salespaymentsreturn_id in (select id from db_salespaymentsreturn where return_id in ($ids))")
		...
		if($q1->num_rows()>0){
			...
			$this->db->where("return_id in ($ids)");
			$this->db->delete("db_salesitemsreturn");

			//DELETE FROM ACCODE TRANSACTIONS
			$this->db->where("ref_salespaymentsreturn_id in (select id from db_salespaymentsreturn where return_id in ($ids))")->delete("ac_transactions");

			$this->db->where("return_id in ($ids)");
			$this->db->delete("db_salespaymentsreturn");
	...
	public function return_sales_list($return_id){
		...
		foreach ($q1->result() as $res1) {
			...
			$item_stock_qty = $this->db->query("select coalesce(stock,0) as stock from db_items where id=".$res1->item_id)->row();
			$item_stock_qty = ($item_stock_qty) ? $item_stock_qty->stock : 0;
			/*end*/
	...
	public function delete_payment_from_account($payment_id){
		...
		//UNLINK FROM ACCOUNT
		$q1=$this->db->query("update db_salespaymentsreturn set account_id=NULL where id='$payment_id'");

		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_salespaymentsreturn_id in ($payment_id)")->delete("ac_transactions");

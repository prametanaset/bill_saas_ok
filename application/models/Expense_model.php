...
class Expense_model extends MY_Model {
	...
	private function _get_datatables_query()
	{
	    ...
	    $expense_from_date = $this->input->post('expense_from_date');
	    if(!empty($expense_from_date)) {
	    	$expense_from_date = system_fromatted_date($expense_from_date);
	    	if($expense_from_date!='1970-01-01'){
	    		$this->db->where("a.expense_date>=",$expense_from_date);
	    	}
	    }

	    $expense_to_date = $this->input->post('expense_to_date');
	    if(!empty($expense_to_date)) {
	    	$expense_to_date = system_fromatted_date($expense_to_date);
	    	if($expense_to_date!='1970-01-01'){
	    		$this->db->where("a.expense_date<=",$expense_to_date);
	    	}
	    }
	...
	public function delete_expenses_from_account($ids){
		$this->db->trans_begin();
      	//ACCOUNT RESET
		...
		//UNLINK FROM ACCOUNT
		$this->db->set("account_id",NULL)->where("id in ($ids)");
		//if not admin
		if(!is_admin()){
			$this->db->where("store_id",get_current_store_id());
		}
		$query1=$this->db->update("db_expense");


		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_expense_id in ($ids)")->delete("ac_transactions");

...
class Subscription_model extends MY_Model {
	...
	public function count_all()
  {
    ...
    $info = array(
      ...
      $this->db->query("ALTER TABLE bill_xml.db_subscription AUTO_INCREMENT = 1");
      $query1 = $this->db->insert('bill_xml.db_subscription', array_merge($info,$info1));
      $this->session->set_flashdata('success', 'Success!! New Subscription Added Successfully!');
    }
    else{
      $query1 = $this->db->where('id',$q_id)->update('bill_xml.db_subscription', $info);
      $this->session->set_flashdata('success', 'Success!! Subscription Updated Successfully!');
    }
    $this->db->set("expire_date",null)->where("expire_date",'1970-01-01')->update("bill_xml.db_subscription");
		if(!$query1){
		  return "failed";
		}
		return "success";
  ...
  public function update_status($id,$status){
	...
	public function delete_subscription_from_table($ids){
      $this->db->trans_begin();
          ...
          $query2=$this->db->delete("bill_xml.db_subscription");
          #---------------------------------

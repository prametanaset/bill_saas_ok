...
class Purchase_model extends MY_Model {
	...
	private function _get_datatables_query()
	{
	    ...
	    $purchase_from_date = $this->input->post('purchase_from_date');
	    if(!empty($purchase_from_date)) {
	    	$purchase_from_date = system_fromatted_date($purchase_from_date);
	    	if($purchase_from_date!='1970-01-01'){
	    		$this->db->where("a.purchase_date>=",$purchase_from_date);
	    	}
	    }

	    $purchase_to_date = $this->input->post('purchase_to_date');
	    if(!empty($purchase_to_date)) {
	    	$purchase_to_date = system_fromatted_date($purchase_to_date);
	    	if($purchase_to_date!='1970-01-01'){
	    		$this->db->where("a.purchase_date<=",$purchase_to_date);
	    	}
	    }
	...
	public function verify_save_and_update(){
		...
		$discount_total = round(floatval($tot_discount_to_all_amt), 2);
		...
		$this->db->update('db_purchase', array(
		...
		if($amount>0 && !empty($payment_type)){
			...
			if($amount>$tot_total_amt){
				echo "จำนวนเงินที่ต้องชำระไม่ควรเกินจำนวนเงินในใบแจ้งหนี้!!";exit;
			}
			...
			*/
			if($command=='update'){
				...
				if(($tot_payment+$amount)>$tot_total_amt){
					echo "จำนวนเงินที่ต้องชำระไม่ควรเกินจำนวนเงินในใบแจ้งหนี้!!\nกรุณาตรวจสอบการชำระเงินครั้งก่อนด้วย.";exit;
				}
			}
	...
	public function delete_payment_from_account($payment_id){
		...
		$purchasepayments = $this->db->query("select purchase_id,supplier_id from db_purchasepayments where id=$payment_id")->row();
		$purchase_id = $purchasepayments->purchase_id;
		$supplier_id = $purchasepayments->supplier_id;

		//UNLINK FROM ACCOUNT
		$q1=$this->db->query("update db_purchasepayments set account_id=NULL where id='$payment_id'");
		
		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_purchasepayments_id in ($payment_id)")->delete("ac_transactions");
	...
	public function delete_purchase($ids){
      	...
      	//ACCOUNT RESET
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_purchasepayments_id in (select id from db_purchasepayments where purchase_id in ($ids))")
									->group_by("debit_account_id,credit_account_id")
									->get("ac_transactions");
		//ACCOUNT RESET END
		...
		if($q8->num_rows()>0){
			$i=1;
			echo "ขออภัย! บันทึกไม่ถูกลบ! มีใบส่งคืนสินค้าจากการซื้อ:";
			foreach($q8->result() as $res){
				echo "<br>".$i++.".ใบส่งคืนสินค้า ตามรหัสการซื้อ :".$res->purchase_code;
			}
			echo "<br>หากต้องการลบการซื้อ! คุณต้องลบใบส่งคืนสินค้าก่อน!";
			exit();
		}
		...
		#----------------------------------
		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_purchasepayments_id in (select id from db_purchasepayments where purchase_id in ($ids))")->delete("ac_transactions");
	...
	public function get_items_info($rowcount,$item_id){
		...
		if(!$res1){ return; } // Validate item exists

		$q3=$this->db->query("select * from db_tax where id=".$res1->tax_id)->row();
		$tax_val = ($q3) ? $q3->tax : 0;
		$tax_name = ($q3) ? $q3->tax_name : '';
		
		$item_tax_amt = ($res1->tax_type=='Inclusive') ? calculate_inclusive($res1->sales_price,$tax_val) :calculate_exclusive($res1->sales_price,$tax_val);
		  ...
		  <div class="modal-dialog ">
		    ...
		    <div class="row">
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" onclick="save_payment(<?=$purchase_id;?>)" class="btn bg-green btn-lg place_order btn-lg payment_save">บันทึก<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
                                ...
                                <tbody>
									...
									if($q1->num_rows()>0){
										foreach ($q1->result() as $res1) {
											...
											echo "<td>
                                                    <a href='".base_url('purchase/print_payment_receipt/'.$res1->id)."' target='_blank' class='pointer btn btn-primary' title='Print Receipt'><i class='fa fa-print'></i></a>
                                                    <a onclick='delete_purchase_payment(".$res1->id.")' class='pointer btn  btn-danger' title='Delete Payment'><i class='fa fa-trash'></i></a>
                                                  </td>";	
											echo "</tr>";
										}
									}
									else{
										echo "<tr><td colspan='7' class='text-danger text-center'>ไม่พบข้อมูล</td></tr>";
									}
									?>
                                </tbody>
                            </table>

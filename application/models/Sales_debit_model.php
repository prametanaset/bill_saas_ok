...
class Sales_debit_model extends MY_Model {
	//Datatable start
	var $table = 'db_salesdebit as a';
	...
	private function _get_datatables_query()
	{
		...
		if(!is_admin()){
	      	if($this->session->userdata('role_id')!='2'){
	      		if(!permissions('show_all_users_sales_debit_invoices')){
	      			$this->db->where("upper(a.created_by)",strtoupper($this->session->userdata('inv_username')));
	      		}
	      	}
	      }

	    $debit_from_date = $this->input->post('debit_from_date');
	    if(!empty($debit_from_date)) {
	    	$debit_from_date = system_fromatted_date($debit_from_date);
	    	if($debit_from_date!='1970-01-01'){
	    		$this->db->where("a.debit_date>=",$debit_from_date);
	    	}
	    }

	    $debit_to_date = $this->input->post('debit_to_date');
	    if(!empty($debit_to_date)) {
	    	$debit_to_date = system_fromatted_date($debit_to_date);
	    	if($debit_to_date!='1970-01-01'){
	    		$this->db->where("a.debit_date<=",$debit_to_date);
	    	}
	    }
	...
	public function verify_save_and_update(){
		...
		validate_package_offers('max_invoices','db_salesdebit');
		//END

		$q1_store=$this->db->query("select * from db_store where status=1 and id=".get_current_store_id());
    	$res1_store=$q1_store->row();

		$this->db->trans_begin();
		$debit_date=system_fromatted_date($debit_date);
	    ...
	    if(isset($debit_id) && !empty($debit_id)){
			//$previous_return=$this->db->query("select item_id,debit_qty from db_salesitemsdebit where debit_id=".$debit_id);
		}
	    ...
	    if($command=='save' || $command=='create'){//Create sales code unique if first time entry
			...
			$this->db->query("ALTER TABLE db_salesdebit AUTO_INCREMENT = 1");
			
		    $sales_entry = array(
		    				'sales_id' 		=> $sales_id,
		    				'count_id' 					=> get_count_id('db_salesdebit'),
		    				'debit_code' 				=> get_init_code('sales_debit'), 
		    				...
		    				'other_charges_type' 		=> $other_charges_type,
		    				/*Discount*/
			...
			$q1 = $this->db->insert('db_salesdebit', array_merge($sales_entry,$sales_entry_init));
			...
			$sales_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
			$sales_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();
			$q1 = $this->db->where('id',$debit_id)->update('db_salesdebit', array_merge($sales_entry,$sales_entry_init));

			##############################################START
			//FIND THE PREVIOUSE ITEM LIST ID'S
			$prev_item_ids = $this->db->select("item_id")->from("db_salesitemsdebit")->where("debit_id",$debit_id)->get()->result_array();
			##############################################END

			$q11=$this->db->query("delete from db_salesitemsdebit where debit_id='$debit_id'");
			if(!$q11){
				return "failed";
			}
		}
		...
		/*if(isset($debit_id) && isset($previous_return)){
			foreach ($previous_return->result() as $row){
					if(!empty($sales_id)){
			         $this->adjust_sales_item($sales_id,$row->item_id,$row->debit_qty,'add_qty_in_update_mode');
					}
			}
		}*/
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$debit_qty			=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_3']));
               ...
               // $discount_input  =(empty($discount_input)) ? 0 : $discount_input;
				//$discount_amt 		=($debit_qty * $unit_total_cost)*$discount_input/100;
				...
				$discount_amt_per_unit = $discount_amt/$debit_qty;
				...
				$q2 = $this->db->insert('db_salesitemsdebit', $salesitems_entry);
				
				/*Find the Item Exist in sales entry or not*/
				//$this->adjust_sales_item($sales_id,$item_id,$debit_qty,$command);
		...
		// Calculate VAT and Taxable Amount for reporting (Sequence A Prorating Logic)
		$this->db->select("a.tax_id, a.tax_type, a.total_cost, b.tax, a.tax_amt");
		$this->db->from("db_salesitemsdebit a");
		$this->db->join("db_tax b", "b.id=a.tax_id", "left");
		$this->db->where("a.debit_id", $debit_id);
		$items_q = $this->db->get();
		...
		foreach($items_q->result() as $row){
			$tax_rate = floatval($row->tax);
			$row_total = floatval($row->total_cost);
			if($tax_rate > $max_tax_rate) { $max_tax_rate = $tax_rate; }
			
			if($tax_rate > 0){
				if($row->tax_type == 'Inclusive'){
					$de_taxed = ($row_total * 100) / (100 + $tax_rate);
					if(!isset($tax_groups[(string)$tax_rate])) $tax_groups[(string)$tax_rate] = 0;
					$tax_groups[(string)$tax_rate] += $de_taxed;
				} else {
					$base = $row_total - floatval($row->tax_amt);
					if(!isset($tax_groups[(string)$tax_rate])) $tax_groups[(string)$tax_rate] = 0;
					$tax_groups[(string)$tax_rate] += $base;
				}
			} else {
		...
		$vat_no = floatval($res1_store->vat_no);

		$other_charges_total = floatval($other_charges_amt);
		if($other_charges_total > 0){
			$oc_tax_rate = 0;
			if(!empty($other_charges_tax_id)){
				$oc_tax_q = $this->db->select("tax")->where("id", $other_charges_tax_id)->get("db_tax");
				if($oc_tax_q->num_rows() > 0) $oc_tax_rate = floatval($oc_tax_q->row()->tax);
			}
			
			if($oc_tax_rate <= 0) {
				if($max_tax_rate > 0) { $oc_tax_rate = $max_tax_rate; }
				else if($vat_no > 0) { $oc_tax_rate = $vat_no; }
			}
			
			if($oc_tax_rate > 0){
				$oc_base_detaxed = ($other_charges_total * 100) / (100 + $oc_tax_rate);
				if(!isset($tax_groups[(string)$oc_tax_rate])) $tax_groups[(string)$oc_tax_rate] = 0;
				$tax_groups[(string)$oc_tax_rate] += $oc_base_detaxed;
			} else {
				$total_exempt_base += $other_charges_total;
			}
		}

		$total_base_sum = $total_exempt_base;
		foreach($tax_groups as $rate => $base) { $total_base_sum += $base; }

		$discount_total = round(floatval($tot_discount_to_all_amt) + floatval($coupon_discount_amt), 2);
        
        // Extract VAT from the discount before proportionally spreading it (Sync with Frontend)
        $discount_base = ($discount_total * 100) / (100 + $max_tax_rate);
		
        $discount_ratio = ($total_base_sum > 0) ? ($discount_base / $total_base_sum) : 0;

		$net_exempt = $total_exempt_base - ($total_exempt_base * $discount_ratio);
		
		$total_net_taxable = 0;
		$total_vat_amount = 0;
		
		foreach($tax_groups as $rate => $base){
			$net_base = $base - ($base * $discount_ratio);
			$rounded_net_base = round($net_base, 2);
			$vat_amt = round(($rounded_net_base * floatval($rate) / 100), 2);
			
			$total_net_taxable += $rounded_net_base;
			$total_vat_amount += $vat_amt;
		}

		$taxable_amount = $total_net_taxable;
		$vat_amt = $total_vat_amount;
		$exempt_amount = round($net_exempt, 2);

		$final_grand_total = $total_net_taxable + $total_vat_amount + $exempt_amount;
		$final_grand_total = round($final_grand_total, 2);

		$this->db->where('id', $debit_id);
		$this->db->update('db_salesdebit', array(
			'grand_total' => $final_grand_total,
			'subtotal'    => $tot_subtotal_amt,
			'other_charges_amt' => $other_charges_amt,
			'tot_discount_to_all_amt' => $tot_discount_to_all_amt,
			'vat'         => 7, // Standard Thai VAT
			'vat_amt'     => $vat_amt,
			'taxable_amount' => $taxable_amount
		));
		
		// Update variables for payment check below
		$tot_total_amt = $final_grand_total;
		...
		if($amount>0 && !empty($payment_type)){
			...
			*/
			if($command=='update'){
				$tot_payment = $this->db->select('coalesce(sum(payment),0) as payment')->where('debit_id',$debit_id)->get('db_salespaymentsdebit')->row()->payment;
			...
			$salespayments_entry = array(
		    		...
		    		'count_id'	  		=> get_count_id('db_salespaymentsdebit'),
					'sales_id' 			=> $sales_id,
					'debit_id' 		=> $debit_id, 
					'payment_date'		=> $debit_date,//Current Payment with sales entry
			...
			$q3 = $this->db->insert('db_salespaymentsdebit', $salespayments_entry);
		...
		$q10=$this->update_sales_payment_status_by_sales_id($debit_id,$customer_id);
		if($q10!=1){
			return "failed";
		}
		...
		if(isset($send_sms) && $customer_id!=1){
			if(send_sms_using_template($debit_id,2)==true){
		...
		##############################################START
		//FIND THE PREVIOUSE ITEM LIST ID'S
		$curr_item_ids = $this->db->select("item_id")->from("db_salesitemsdebit")->where("debit_id",$debit_id)->get()->result_array();
		$two_array = array_merge($prev_item_ids,$curr_item_ids);

        /* COMBO: Add components to update list */
        if(!empty($two_array)){
            $start_ids = array_map(function($a){ return $a['item_id']; }, $two_array); 
            $extra_ids = array();
            if(!empty($start_ids)){
                $combos = $this->db->where_in('parent_id', $start_ids)->get('db_combo_items')->result();
                foreach($combos as $c){
                    $extra_ids[] = array('item_id' => $c->item_id);
                }
            }
            $two_array = array_merge($two_array, $extra_ids);
        }
		...
		return "success<<<###>>>$debit_id";
        } catch (\Throwable $e) {
            http_response_code(500);
            echo "Throwable caught: " . $e->getMessage() . " on line " . $e->getLine() . " in file " . basename($e->getFile());
            exit;
        }
	...
	function update_sales_payment_status_by_sales_id($debit_id,$customer_id){
	//UPDATE PRODUCTS QUANTITY IN PRODUCTS TABLE
		
		$q8=$this->db->query("select COALESCE(SUM(payment),0) as payment from db_salespaymentsdebit where debit_id='$debit_id'");
		$sum_of_payments=$q8->row()->payment;

		$q9=$this->db->query("select coalesce(sum(grand_total),0) as total from db_salesdebit where id='$debit_id'");
		...
		$q7=$this->db->query("update db_salesdebit set 
		...
		$q12 = $this->db->query("update db_customers set sales_debit_due=(select COALESCE(SUM(grand_total),0)-COALESCE(SUM(paid_amount),0) from db_salesdebit where customer_id='$customer_id') where id=$customer_id");
		if(!$q7)
		{
			return false;
		}
	...
	function update_sales_payment_status($debit_id=null,$customer_id=null){
	//UPDATE PRODUCTS QUANTITY IN PRODUCTS TABLE

		if(empty($debit_id)){ //If sales ID not exist you need setup all the customers sales due
			$q11=$this->db->query("select id from db_customers");
			if($q11->num_rows()>0){
				foreach ($q11->result() as $res) {
					$q12=$this->db->query("select id from db_salesdebit where customer_id=".$res->id);
					if($q12->num_rows()>0){
						...
						$q13=$this->db->query("update db_customers set sales_debit_due=0 where id=".$res->id);
						if(!$q13){
							return false;
						}
					}
			...
			return true;
					...
					if(!$this->update_sales_payment_status_by_sales_id($debit_id,$customer_id)){
						return false;
					}
					return true;
		}
	...
	public function update_status($id,$status){
        ...
        $query1="update db_salesdebit set status='$status' where id=$id";
        if ($this->db->simple_query($query1)){
            echo "success";
        }
        else{
	...
	public function update_return_bit(){
		// Logic removed to prevent conflict with Sales Return labeling.
		// Sales Debit uses dynamic subquery in Sales_model.php for its label.
	}
	...
	public function delete_return($ids){
		...
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_salespaymentsdebit_id in (select id from db_salespaymentsdebit where debit_id in ($ids))")
		...
		//ACCOUNT RESET END
      	...
      	##############################################START
		//FIND THE PREVIOUSE ITEM LIST ID'S
		$prev_item_ids = $this->db->select("item_id")->from("db_salesitemsdebit")->where("debit_id in ($ids)")->get()->result_array();
		##############################################END

		//find customer list group by
		$this->db->select("customer_id,id as debit_id");
		$this->db->where("id in ($ids)");
		$this->db->where("store_id",get_current_store_id());
		$this->db->group_by("customer_id");
		$customer_records=$this->db->get("db_salesdebit");
		//end

		

      	//If you are editing the return products.
		/*$q1=$this->db->query("select item_id,debit_qty,sales_id from db_salesitemsdebit where debit_id in($ids)");
		if($q1->num_rows()>0){
			foreach ($q1->result() as $row){
					
					if(!empty($row->sales_id)){
			         $this->adjust_sales_item($row->sales_id,$row->item_id,$row->debit_qty,'add_qty_in_update_mode');
					}
			}
		}*/
			#----------------------------------
			...
			$q3=$this->db->delete("db_salesdebit");

			// DELETE orphaned child records to prevent inheritance if IDs are reused
			$this->db->where("debit_id in ($ids)");
			$this->db->delete("db_salesitemsdebit");

			//DELETE FROM ACCODE TRANSACTIONS
			$this->db->where("ref_salespaymentsdebit_id in (select id from db_salespaymentsdebit where debit_id in ($ids))")->delete("ac_transactions");

			$this->db->where("debit_id in ($ids)");
			$this->db->delete("db_salespaymentsdebit");
			#----------------------------------
		...
		if($customer_records->num_rows()>0){
        	foreach ($customer_records->result() as $res) {
        		if(!$this->update_sales_payment_status($res->debit_id,$res->customer_id)){
		        	return 'failed';
		        }
        ...
        if(!empty($prev_item_ids)){
            $start_ids = array_map(function($a){ return $a['item_id']; }, $prev_item_ids); 
            $extra_ids = array();
            if(!empty($start_ids)){
                $combos = $this->db->where_in('parent_id', $start_ids)->get('db_combo_items')->result();
                foreach($combos as $c){
            ...
            $prev_item_ids = array_merge($prev_item_ids, $extra_ids);
        }
	...
	public function get_items_info($rowcount,$item_id){
	    ...
	    $invoice_sales_qty=0;
	    if(!empty($sales_id)){
	      // For debit notes, we don't restrict to items already in the invoice.
	      $q_item = $this->db->query("select coalesce(sum(sales_qty),0) as sales_qty from db_salesitems where item_id=$item_id and sales_id=$sales_id");
	      if($q_item->num_rows() > 0) {
	          $invoice_sales_qty = $q_item->row()->sales_qty;
	      }
	    }
		...
		$item_tax_amt = ($res1->tax_type=='Inclusive') ? calculate_inclusive($res1->sales_price,$q3->tax) :calculate_exclusive($res1->sales_price,$q3->tax);
			...
			$info = array(
							...
							'item_id' 					=> $res1->id, 
							'item_sales_price' 			=> ($res1->tax_type=='Inclusive') ? $res1->sales_price : $res1->price, // Exclusive should use base price
							'item_tax_id' 				=> $q3->id, 
							'item_sales_qty' 			=> 1, 
							'description' 				=> '', 
	...
	public function sales_list($sales_id){
		...
		foreach ($q1->result() as $res1) {
			...
			//Validate Item
			if(!$res2){
			    //Try to get info from salesitems if item deleted from master
			    $item_name = "Item Deleted (ID:".$res1->item_id.")";
			    $item_price = $res1->price_per_unit; //Fallback
			}
			else{
			...
			if(!$q3){
			    ...
			    $tax = $q3->tax;
			    $tax_name = $q3->tax_name;
			}

			$info = array(
							'item_tax' 					=> $tax, 
							'item_tax_name' 			=> $tax_name, 
							'item_name' 				=> $item_name,
	...
	public function return_sales_list($debit_id){
		$db_name = $this->db->query("SELECT database() as db")->row()->db;
		log_message('error', 'DEBUG: Sales_debit_model::return_sales_list. DB: ' . $db_name . ' for ID: ' . $debit_id);
		$q1=$this->db->select('*')->from('db_salesitemsdebit')->where("debit_id=$debit_id")->get();
		log_message('error', 'DEBUG: Sales_debit_model found ' . $q1->num_rows() . ' items');
		$rowcount =1;
		$result = '';
		foreach ($q1->result() as $res1) {
			...
			$item_sales_qty=0;
			 if(!empty($res1->sales_id)) { 
                $q_sales = $this->db->query("select coalesce(sales_qty,0) as sales_qty from db_salesitems where sales_id=".$res1->sales_id.' and item_id='.$res1->item_id);
		 	    $item_sales_qty = ($q_sales->num_rows() > 0) ? $q_sales->row()->sales_qty : 0;
		 	} 
			$q_stock = $this->db->query("select coalesce(stock,0) as stock from db_items where id=".$res1->item_id);
			$item_stock_qty = ($q_stock->num_rows() > 0) ? $q_stock->row()->stock : 0;
			/*end*/
			
			//$info['item_available_qty'] = ($item_stock_qty<$item_sales_qty) ? $item_stock_qty+$res1->debit_qty : $item_sales_qty+$res1->debit_qty;	
			//NEW
			$item_available_qty = (!empty($res1->sales_id)) ? $item_sales_qty : $item_stock_qty+$res1->debit_qty;
	...
	public function return_row_with_data($rowcount,$info){
		extract($info);
		$item_amount = ($item_tax_type=='Inclusive') ? ($item_sales_price * $item_sales_qty) : ($item_sales_price * $item_sales_qty) + ($item_tax_amt * $item_sales_qty);
		$item_tax_amt_row = $item_tax_amt * $item_sales_qty;
		ob_start();
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <td id="td_<?=$rowcount;?>_3">
                  <div class="input-group ">
                     ...
                     <input type="text" value="<?=format_qty($item_sales_qty);?>" class="form-control no-padding text-center" onkeyup="calculate_tax(<?=$rowcount;?>)" id="td_data_<?=$rowcount;?>_3" name="td_data_<?=$rowcount;?>_3">
                     <span class="input-group-btn">
               ...
               <td id="td_<?=$rowcount;?>_11">
                  <input type="text" name="td_data_<?=$rowcount;?>_11" id="td_data_<?=$rowcount;?>_11" class="form-control text-right no-padding only_currency text-center" value="<?=store_number_format($item_tax_amt_row,0);?>" readonly>
               </td>
	...
	public function delete_payment_from_account($payment_id){
        $this->db->trans_begin();
         //ACCOUNT RESET
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_salespaymentsdebit_id in ($payment_id)")
									->group_by("debit_account_id,credit_account_id")
									->get("ac_transactions");
		//ACCOUNT RESET END

		$returnpayments = $this->db->query("select debit_id,customer_id from db_salespaymentsdebit where id=$payment_id")->row();
		$debit_id = $returnpayments->debit_id;
		$customer_id = $returnpayments->customer_id;

		//UNLINK FROM ACCOUNT
		$q1=$this->db->query("update db_salespaymentsdebit set account_id=NULL where id='$payment_id'");

		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_salespaymentsdebit_id in ($payment_id)")->delete("ac_transactions");

		//ACCOUNT RESET
        if($reset_accounts->num_rows()>0){
        	foreach ($reset_accounts->result() as $res1) {
        		if(!empty($res1->debit_account_id)){
	        		if(!update_account_balance($res1->debit_account_id)){
						return 'failed';
					}
				}

				if(!empty($res1->credit_account_id)){
					if(!update_account_balance($res1->credit_account_id)){
						return 'failed';
					}
				}
	...
	public function delete_payment($payment_id){
		...
		//$debit_id = $this->db->query("select debit_id from db_salespaymentsdebit where id=$payment_id")->row()->debit_id;
		...
		$q1=$this->db->query("delete from db_salespaymentsdebit where id='$payment_id'");

		//DELETE FROM ACCODE TRANSACTIONS
		$this->db->where("ref_salespaymentsdebit_id in ($payment_id)")->delete("ac_transactions");
		$q2=$this->update_sales_payment_status($debit_id,$customer_id);
	...
	public function show_pay_now_modal($debit_id){
		$q1=$this->db->query("select * from db_salesdebit where id=$debit_id");
		$res1=$q1->row();
	    ...
	    $debit_date=$res1->debit_date;
	    $reference_no=$res1->reference_no;
	    $debit_code=$res1->debit_code;
	    $debit_note=$res1->debit_note;
		  ...
		  <div class="modal-dialog ">
		    ...
		    <div class="row">
		      <div class="col-md-12">
		      	<div class="row invoice-info">
			        ...
			        <div class="col-sm-4 invoice-col">
			          ...
			          <address>
			            <b><?= $this->lang->line('invoice'); ?> #<?php echo  $debit_code; ?></b><br>
			            <b><?= $this->lang->line('date'); ?> :<?php echo show_date($debit_date); ?></b><br>
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" onclick="save_payment(<?=$debit_id;?>)" class="btn bg-green btn-lg place_order btn-lg payment_save">บันทึก<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
	...
	public function save_payment(){
		...
		if($amount>0 && !empty($payment_type)){
			...
			$salespayments_entry['store_id']=$this->db->select("store_id")->where('id',$debit_id)->get('db_salesdebit')->row()->store_id;
			$q3 = $this->db->insert('db_salespaymentsdebit', $salespayments_entry);
		...
		$q10=$this->update_sales_payment_status($debit_id,$customer_id);
		if($q10!=1){
			return "failed";
		}
		return "success";
	...
	public function view_payments_modal($debit_id){
		$q1=$this->db->query("select * from db_salesdebit where id=$debit_id");
		$res1=$q1->row();
		$customer_id = $res1->customer_id;
		$q2=$this->db->query("select * from db_customers where id=$customer_id");
		              ...
		              <div class="row">
		         		<div class="col-md-12">
		                      ...
		                      <table class="table table-bordered">
                                ...
                                <tbody>
                                	<?php
                                	$q1=$this->db->query("select * from db_salespaymentsdebit where debit_id=$debit_id");
									$i=1;
									$str = '';
									if($q1->num_rows()>0){
										foreach ($q1->result() as $res1) {

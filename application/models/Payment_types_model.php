...
class Payment_types_model extends MY_Model {
	...
	public function verify_and_save(){
		...
		if($query->num_rows()>0){
			return "ชื่อประเภทการชำระเงินนี้มีอยู่ในระบบแล้ว.";
			...
			if ($q1){
					$this->session->set_flashdata('success', 'สำเร็จ!! บันทึกข้อมูลเรียบร้อยแล้ว!');
			        return "success";
			}
			else{
			        return "failed";
	...
	public function update_payment_type(){
		...
		if($query->num_rows()>0){
			...
			if ($q1){
					$this->session->set_flashdata('success', 'สำเร็จ!! อัปเดตข้อมูลเรียบร้อยแล้ว!');
			        return "success";
			}
			else{
			        return "failed";
	...
	public function delete_payment_type($id){
		...
		if($tot> 0){
			return "Sorry! Can't Delete,<br>Payment Type already used in Sales Payment Receive!";
		}*/

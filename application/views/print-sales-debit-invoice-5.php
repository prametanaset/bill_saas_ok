...
<?php
     ...
     $q3=$this->db->query("SELECT b.coupon_id,b.coupon_amt,b.store_id,b.sales_id,a.customer_name,a.mobile,a.phone,a.gstin,a.tax_number,a.email,
                            ...
                            b.debit_code,b.debit_status,b.debit_note,b.taxable_amount,b.vat_amt,b.vat,
     ...
     a.postcode,a.address,b.sales_date,b.created_time,b.reference_no,b.total_amount,
   ...
   */
        ...
        $res10x = $q10x->row();
     if ($res10x) {
         $previous_total_amount = $res10x->total_amount;
         $vat_sale = $res10x->vat;
     } else {
         $previous_grand_total = 0;
         $vat_sale = 0;
...
<span style="color:#0070C0; font-size:34px; font-weight: 800;">
    <?= $this->lang->line('sale_debit_invoice') ?>
  </span>
</div>
...
<!-- <table align="center" width="100%" height='100%'   > -->
    ...
    <thead>
          ...
          <table width="100%" height='100%' class="style_hidden fixed_table">
           <tr>
             ...
             <td colspan="10">                    
                ...
                <p style="font-size: 18px; line-height: 1.1;">  <span style="font-size: 20px;"><?php echo $store_name; ?></span> <br> 
                     ...
                     <span>
                      <table style="width: 100%;" class="style_hidden fixed_table">
                        ...
                        <td colspan="6">
                            <?= $this->lang->line('date'); ?> :
                            <span style="font-size: 20px;">
                              <b><?php echo show_date($debit_date); ?></b>
                            </span>
                          </td>
                        </tr>
              ...
              <tr>
                ...
                <td colspan="10">              
                 <p style="font-size: 18px; line-height: 1.1;">  
                   <span style="font-size: 18px;"><?= $this->lang->line('customer'); ?> :</span>      <br>                             
                   <span style="font-size: 20px;"><?php echo $customer_name; ?></span> <br> 
                        ...
                        <tr>
                          ...
                          <?php if(!empty($sales_code)) { ?>
                                 <label for="" class="col-sm-2 control-label"><?= $this->lang->line('returned_against_sales_invoices'); ?><label class="text-danger"></label> </label> 
                                  <label class="col-sm-3 control-label" style="text-align: left; font-size: 20px;"> : <?= $sales_code;?></label>
                                <?php } ?>
                          </td>                       
                        </tr>
                       ...
                       <tr>                        
                         <td colspan="6">
                          <div class="form-group">   
                             ...
                             <label for="debit_note" class="col-sm-4 control-label" style="font-size: 18px;"><?= $this->lang->line('debit_note'); ?></label>    
                             <span style="font-size: 22px;">  
                               <label class="control-label " style="font-size: 20px;">: <?= $debit_note;?></label>
                              </span>                                                          
                         </div>
                         </td>                      
                       </tr>  
      ...
      <tr style="background-color: #dbe0e6 !important; color: #333333 !important; border: none !important;"><!-- Colspan 10 -->
        ...
        <th colspan='3' class="text-center" style="border: none; padding: 10px 5px;"><?= $this->lang->line('total_amount'); ?></th>
      </tr>
  </thead>
    ...
    <tbody style="border-bottom: 2px solid #0070C0;">
      ...
      <?php
              ...
              $q2 = $this->db->select('a.description,c.item_name, a.debit_qty,
                                  a.price_per_unit, b.tax,b.tax_name,a.tax_amt,
                                  a.discount_input,a.discount_amt, a.unit_total_cost,
                                  a.total_cost , d.unit_name,c.hsn')
                        ->from("db_salesitemsdebit AS a")
                        ->where("a.debit_id",$debit_id)
              ...
              foreach ($q2->result() as $res2) {
                  ...
                  $before_tax=$res2->unit_total_cost;// * $res2->debit_qty;
                  $tot_cost_before_tax=$before_tax * $res2->debit_qty;
              ...
              //    echo "<td colspan='2' class='text-left'>".$res2->hsn."</td>";
                  echo "<td colspan='1' class='text-center'>".format_qty($res2->debit_qty)."</td>";
                  ...
                  echo "<td colspan='2' class='text-center'>" . $res2->tax_name . " ".store_number_format($res2->tax)."% " ."</td>";  
      ...
      <tr>
       ...
       <tr>
    <td colspan="13" style="text-align: right;"><b><?= $this->lang->line('other_charges'); ?></b></td>
    <td colspan="3" style="text-align: right;" ><b><?php echo store_number_format($other_charges_amt); ?></b></td>
    </tr>
    <?php } ?>

      <?php if (!empty($tot_discount_to_all_amt != 0)) { ?>
        <tr>
          <td colspan="13" class='text-right'><b><?= $this->lang->line('discount_on_all'); ?></b>      
          </td>
      ...
      <?php } ?>
           ...
           <tr>
            ...
            <tr>
                ...
                <td colspan="3" class='text-right' ><b><?php echo store_number_format( $previous_total_amount); ?></b> </td>
            </tr>
           
            <tr>
                <td colspan="13" class='text-right'><b><?= $this->lang->line('total_correct_amt'); ?></b></td>
                <td colspan="3" class='text-right' ><b><?php echo store_number_format( $reference_no); ?></b> </td>
            </tr>  
            <tr>
              <td colspan="13" class='text-right'><b><?= $this->lang->line('difference_debit'); ?></b></td>
              <td colspan="3" class='text-right' ><b><?php echo store_number_format( $total_non_tax + $db_taxable_amount); ?></b> </td>
            </tr>
     ...
     <?php if(!empty( $total_non_tax !=0)) {?>
        <tr>
          <td colspan="13" class='text-right'><b><?= $this->lang->line('total_zero_vat_diff'); ?></b></td>
          <td colspan="3" class='text-right'><b><?php echo store_number_format( $total_non_tax); ?></b></td>
        </tr>
      <?php } ?>

       <?php if(!empty($db_taxable_amount !=0)) {?>
      <tr>
        <td colspan="13" class='text-right'><b><?= $this->lang->line('total_taxable_diff'); ?></b></td>
        <td colspan="3" class='text-right'><b><?php echo store_number_format( $db_taxable_amount); ?></b></td>
      </tr>
       <?php } ?>
      ...
      <tr>
        ...
        <td colspan="3" class='text-right'><b><?php echo store_number_format( $db_vat_amt); ?></b> </td>
      </tr>
     <?php } ?>
            ...
            ?>
                ...
                <td colspan="3" class='text-right' style="padding: 5px;"><b><?= $this->lang->line('grand_total'); ?></b></td>
                <td colspan="3" class='text-right' style="padding: 5px;"><b><?php echo store_number_format($grand_total); ?></b> </td>
            </tr>
                  ...
                  <span>
                    <table style="width: 100%;" class="style_hidden fixed_table">
                          ...
                          <td colspan='3' style="height:60px;">
                            <span><b><?= $this->lang->line(' '); ?></b></span></br>
                           ...
                           <td colspan='13'>
                            <div>
                            <span><b> <?= $this->lang->line('etax_invoice'); ?> </b></span>

...
<div class="wrapper">
  ...
  <?php 
      ...
      $total_invoice = $this->db->select("COUNT(*) as total")->from("db_salesdebit")->where("store_id", get_current_store_id())->get()->row()->total;
      ...
      $sal_total = $this->db->select("COALESCE(sum(grand_total),0) AS tot_sal_grand_total")->from("db_salesdebit")->where("store_id", get_current_store_id())->get()->row()->tot_sal_grand_total;
      ...
      $sal_return_total = $this->db->select("COALESCE(SUM(grand_total),0) AS sal_total")->from("db_salesdebit")->where("store_id", get_current_store_id())->get()->row()->sal_total;

  
      //Sales Debit Paid Total
      if (!is_admin()) {
        if ($this->session->userdata('role_id') != '2') {
          $this->db->where("upper(created_by)", strtoupper($this->session->userdata('inv_username')));
        }
      }
      $paid_amount = $this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount")->from("db_salesdebit")->where("store_id", get_current_store_id())->get()->row()->paid_amount;
      ...
      <div class="row">
        ...
        <div class="col-md-3 col-sm-3 col-xs-12">
          <div class="info-box " >
            ...
            <div class="info-box-content">
              <span class="info-box-text" ><?=$CI->currency($sal_total);?></span>
        ...
        <div class="col-md-3 col-sm-3 col-xs-12">
          <div class="info-box " >
            ...
            <div class="info-box-content">
              <span class="info-box-text" ><?=$CI->currency($sales_due_total);?></span>
              <span class="info-box-number"><?=$this->lang->line('total_sales_debit_due');?></span>
            </div>
            <!-- /.info-box-content -->
          </div>
          <!-- /.info-box -->
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header with-border">
              ...
              <div class="box-tools">
                <a class="btn btn-block btn-info" href="<?php echo $base_url; ?>sales_debit/create">
            ...
            <div class="box-body">
              <div class="row">
                <div class="col-md-3">
                  <div class="form-group">
                    <label for="debit_from_date"><?= $this->lang->line('from_date'); ?> </label>
                    <div class="input-group date">
                      <div class="input-group-addon">
                      <i class="fa fa-calendar"></i>
                      </div>
                      <input type="text" class="form-control pull-right datepicker" id="debit_from_date" name="debit_from_date" onchange="load_datatable_with_destroy()">
                    </div>
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="form-group">
                    <label for="debit_to_date"><?= $this->lang->line('to_date'); ?> </label>
                    <div class="input-group date">
                      <div class="input-group-addon">
                      <i class="fa fa-calendar"></i>
                      </div>
                      <input type="text" class="form-control pull-right datepicker" id="debit_to_date" name="debit_to_date" onchange="load_datatable_with_destroy()">
                    </div>
                  </div>
                </div>
              </div>
              <table id="example2" class="table table-bordered custom_hover" width="100%">
                ...
                <tr>
                  ...
                  <th  class='text-center'><?= $this->lang->line('debit_code'); ?></th>
                  <th  class='text-center'><?= $this->lang->line('debit_note'); ?></th>
                  <th  class='text-center'><?= $this->lang->line('difference_debit'); ?></th>
                  <th  class='text-center'><?= $this->lang->line('customer_name'); ?></th>
                  <!-- <th>Warehouse</th> -->
                  <th  class='text-center'><?= $this->lang->line('total_correct_amt'); ?></th>
                  <th  class='text-center'><?= $this->lang->line('paid_amount'); ?></th>
                  <th  class='text-center'><?= $this->lang->line('payment_status'); ?></th>
...
<script type="text/javascript">
      function load_datatable(){
       ...
       /* dom:'<"row"<"col-sm-12"<"pull-left"B><"pull-right">>> <"row margin-bottom-12"<"col-sm-12"<"pull-left"l><"pull-right"fr>>>tip',*/
            buttons: {
              ...
              "ajax": {
                  "url": "<?php echo site_url('sales_debit/ajax_list')?>",
                  "type": "POST",
                  "data": function (d) {
                      d.warehouse_id = $("#warehouse_id").val();
                      d.debit_from_date = $("#debit_from_date").val();
                      d.debit_to_date = $("#debit_to_date").val();
                    },
      ...
      $("#debit_from_date, #debit_to_date").on("change",function(){
          load_datatable_with_destroy();
      });
</script>
<script src="<?php echo $theme_link; ?>js/sales-debit.js?v=1"></script>
<script type="text/javascript">

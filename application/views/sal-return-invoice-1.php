            ...
            <!-- /.col -->
             <div class="col-sm-4 invoice-col">
                   ...
                   <b><?= $this->lang->line('return_status'); ?> : <?php echo  $return_status; ?></b><br>
                    ...
                    <?php if ($sales_code) { ?>
                   <b><?= $this->lang->line('returned_against_sales_invoices'); ?>  :   <?php echo  $sales_code; ?></b><br>
                   <?php } ?>
                      ...
                      <td colspan="6">
                        ...
                        <span style="font-size: 18px;">:
                          <b><?php 
                            ...
                            $q_ref = $this->db->query("SELECT sales_date, grand_total FROM db_sales WHERE sales_code=? LIMIT 1", array($ref_no_clean));
                            if($q_ref->num_rows() > 0){
                                $ref_date = show_date($q_ref->row()->sales_date);
                                $ref_total = $q_ref->row()->grand_total;
                            } else {
                        ...
                        <br>
                        <b><?= $this->lang->line('total_against_sales_invoices'); ?> : <?php echo store_number_format($ref_total); ?></b><br>
                        <?php } ?>
                      </td>
                      </tr>   
                      <br>
            ...
            <div class="col-xs-12 table-responsive">
              <table class="table  records_table table-bordered">
                ...
                if($vat_no == 0) {
                  ...
                  //-----------------------------------//
                 $percent_1 = ($tot_total_cost / 100); // คำนวน มูลค่ารวม 1 %   -----(ก่อนหักส่วนลด)          
                $disc_per = ($tot_discount_to_all_amt / $percent_1);        // คำนวนส่วนลดเป็นเปอร์เซ็นต์  
                //------------------------------------//
                $percent_non_tax = ($total_non_tax / $percent_1);   // คำนวน % ของมูลค่าที่ไม่มี/ยกเว้นภาษี
                $percent_in_tax = ($total_in_tax / $percent_1);    // คำนวน % ของมูลค่าที่รวมภาษี
                //-----------------------------------//

              
                $total_tax_zero_amt = ($grand_total / 100) *  $percent_non_tax;  //---4-----มูลค่าที่ไม่มี/ยกเว้นภาษี (+บริการ/หลังส่วนลด)

                $total_taxable = ($grand_total / 100) *  $percent_in_tax;  //--5-- มูลค่าคำนวน VAT

            
                $vat = $total_taxable / ( 100 + $vat_no);   // จำนวนเงิน vat 1%   -----(หลังหักส่วนลด)
                $vat_amt  = $vat *  $vat_no ;      // มูลค่าภาษีมูลค่าเพิ่ม                
                                                                                        
                $total_vatable =   $total_taxable - $vat_amt ;   //--5-- มูลค่าคำนวน VAT
            ...
            <div class="col-md-6">
              ...
              <div class="row">
               ...
               <div class="row">
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    <label for="return_note" class="col-sm-4 control-label" style="font-size: 17px;"><?= $this->lang->line('total_correct_amt'); ?></label>
                    <div class="col-sm-8">
                      <label class="control-label  " style="font-size: 17px;">: <?= store_number_format(string_to_number($reference_no)); ?></label>
                    </div>
                  </div>
                </div>
                   ...
                   <!--   <?php if(!empty($tot_price_zero_vat !=0)) {?>
                       <tr>
                          ...
                          <th class="text-right" style="padding-left:10%;font-size: 11px;">
                             <h4><b id="" name=""><?php echo store_number_format($total_non_tax);?></b></h4>
                          </th>
                       </tr>
                       <?php } ?> 
                       ...
                       <?php if(!empty( $total_in_tax !=0)) {?> 
                        <tr>
                          ...
                          <th class="text-right" style="padding-left:10%;font-size: 17px;">
                             <h4><b id="subtotal_amt" name="subtotal_amt"><?=store_number_format( $total_in_tax );?></b></h4>
                          </th>
                       </tr>
                        <?php } ?> -->
                    ...
                    <?php if(!empty($other_charges_amt !=0)) {?>
                     ...
                     <?php if(!empty( $total_tax_zero_amt !=0)) {?> 
                       <tr class='text'>
                          <th class="text-right" style="font-size: 17px;"><?= $this->lang->line('total_zero_vat'); ?></th>
                          <th class="text-right" style="padding-left:10%;font-size: 17px;">
                             <h4><b id="subtotal_amt" name="subtotal_amt"><?php echo store_number_format( $total_tax_zero_amt );?></b> </h4>
                          </th>
                       </tr>
                       <?php } ?> 
                       ...
                       <tr class='text'>
                          <th class="text-right" style="font-size: 17px;"><b><?= $this->lang->line('before_amt') ; ?></b></th>
                            <th class="text-right" style="padding-left:10%;font-size: 17px;">
                            <h4><b id="total_amt" name="total_amt"><?=store_number_format($total_vatable );?></b></h4>
                            </th>
                        </tr> 
                       <tr class='text'>
                          <th class="text-right" style="font-size: 17px;"><b><?= $this->lang->line('tax_amount') . store_number_format($vat_no)." % "; ?></b></th>
                            <th class="text-right" style="padding-left:10%;font-size: 17px;">
                            <h4><b id="total_amt" name="total_amt"><?=store_number_format($vat_amt);?></b></h4>
                            </th>
                        </tr> 
                    ...
                    <tr class='text-primary'>
                          <th class="text-right" style="font-size: 17px;"><b><?= $this->lang->line('total_against_sales_invoices') ; ?></b></th>
                            <th class="text-right" style="padding-left:10%;font-size: 17px;">
                            <h3><b id="total_amt" name="total_amt"><?=store_number_format($ref_total );?></b></h3>
                            </th>
                        </tr> 
                       <tr class='text-primary'>
                          <th class="text-right" style="font-size: 17px;"><b><?= $this->lang->line('total_correct_amt') ; ?></b></th>
                            <th class="text-right" style="padding-left:10%;font-size: 17px;">
                              <h3><b id="total_amt" name="total_amt"><?= store_number_format($ref_total - $grand_total); ?></b></h3>
                            </th>
                        </tr> 


                      <tr class='text-primary'>
                        <th class="text-right" style="font-size: 17px; "><?= $this->lang->line('difference_credit'); ?></th>
                        <th class="text-right" style="padding-left:10%;font-size: 17px;">
                          <h3><b id="total_amt" name="total_amt"><?= store_number_format($grand_total); ?></b></h3>
                        </th>
                      </tr>
         ...
         <tr style="margin-top: 5 px">
           ...
           <tr style="margin-top: 5 px">
            ...
            <a href="javascript:void(0);" onclick="printTemplate('<?php echo $base_url; ?>sales_return/print_invoice/<?php echo $return_id; ?>')" class="btn btn-primary">
              <i class="fa fa-print"></i> พิมพ์ใบคืนสินค้า
            </a>
            
          <a href="<?php echo $base_url; ?>sales_return/pdf_a3/<?php echo  $return_id ?>" target="_blank" class="btn btn-primary">
            <i class="fa fa-file-pdf-o"></i> Credit Note           
           </a>

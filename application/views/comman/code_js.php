...
<script>
  $(document).ready(function() {
      // Only run polling if we are logged in as Admin (optional check, but good for performance)
      // For now, we assume this file is only included in Admin pages.
      
      var last_check_time = '';
      var check_interval = 10000; // 10 seconds

      // Initial call to sync time
      check_notifications();

      setInterval(check_notifications, check_interval);

      function check_notifications() {
          // Use base_url defined in code_js.php variable 'base_url'
          var url = base_url + "dashboard/get_notification_counts";
          ...
          $.ajax({
              ...
              success: function(response) {
                  if (response.server_time) {
                      last_check_time = response.server_time;
                  }

                  if (response.notifications && response.notifications.length > 0) {
                      $.each(response.notifications, function(index, note) {
                          if (note.type == 'success') {
                              toastr.success(note.message, 'Payment Received', {
                                  "closeButton": true,
                                  "debug": false,
                                  "newestOnTop": false,
                                  "progressBar": true,
                              ...
                              toastr.info(note.message, 'New Request', {
                                  "closeButton": true,
                                  "debug": false,
                                  "newestOnTop": false,
                                  "progressBar": true,
                          ...
                          if(typeof success_sound !== "undefined"){
                            var playPromise = success_sound.play();
              ...
              // Silence errors for global polling to avoid console spam on pages that might behave differently
          });
      }
  });
</script>

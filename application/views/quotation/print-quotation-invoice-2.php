...
<table autosize="1" style="overflow: wrap" id='mytable' align="center" width="100%" height='100%' cellpadding="0" cellspacing="0">
              ...
              <td colspan="6" rowspan="1">
                <span>
                  <table style="width: 100%;" class="style_hidden fixed_table">
                    <tr>
                      <td colspan="6" style="color:blue; font-size:36px; font-weight: 700;">
                    ...
                    <tr>
                      <td colspan="6">
                        ...
                        <span style="font-size: 18px;">
                          <b>No. :<?php echo " $quotation_code"; ?></b>
                        </span>
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td colspan="6">
                        <?php echo $this->lang->line('expire_date'); ?>:
                        <span style="font-size: 18px;">
                    ...
                    <tr>
                      <td colspan="6">
                        <?php echo $this->lang->line('reference_no'); ?>:
                        <span style="font-size: 18px;">
            ...
            <tr>
             ...
             <!-- <td colspan="6">
                <?php if(!empty($store_logo)) { ?>
                  <center><img src="<?= base_url($store_logo);?>" width="45%"></center>
                <?php } ?>
              </td>-->
            </tr>
          </table>
  ...
  <tbody style="border-bottom: 2px solid #0070C0;">
    <?php
      ...
      $this->db->select("a.description,c.item_name, a.quotation_qty, a.price_per_unit, b.tax, b.tax_name, a.tax_amt, a.discount_amt, a.unit_total_cost, a.total_cost, d.unit_name");
      ...
      foreach ($q2->result() as $res2) {
          ...
          echo "</td>";
          echo "<td colspan='1' class='text-center'>".format_qty($res2->quotation_qty)."</td>";
          echo "<td colspan='2' class='text-center'>".store_number_format($res2->price_per_unit)."</td>";
          echo "<td colspan='2' class='text-center'>".store_number_format($discount_amt)."</td>";         
          echo "<td colspan='2' class='text-center'>".store_number_format($res2->tax_amt)."</td>";
          ...
          if ($res2->tax == 0) { $tot_price_zero_vat += $res2->total_cost; }
      }

      $total_non_tax = $tot_price_zero_vat;
      $total_in_tax = $tot_total_cost - $tot_price_zero_vat;
      $vat_no = floatval($vat_sale);
      if($vat_no == 0 && $total_in_tax != 0) {
          $tax_no = ($total_in_tax - $tot_tax_amt) / 100;
          $vat_no = ($tax_no != 0) ? $tot_tax_amt / $tax_no : 0;
      }

      $percent_all = ($tot_total_cost + $other_charges_amt) / 100;
      $disc_per = ($percent_all != 0) ? $tot_discount_to_all_amt / $percent_all : 0;
      $total_tax_zero_amt = $total_non_tax - (($total_non_tax / 100) * $disc_per);
      $taxable = ($total_in_tax + $other_charges_amt) - ((($total_in_tax + $other_charges_amt) / 100) * $disc_per);
      $vat_amt = ($taxable * $vat_no) / (100 + $vat_no);
      $total_vatable = $taxable - $vat_amt;
    ?>
    ...
    <tr style="background-color: #eaeff5 !important; border-bottom: 1px solid #0070C0;">
      ...
      <td colspan="2" class='text-center' style="padding: 5px;"><b><?php echo store_number_format($tot_discount_amt); ?></b></td> 
      <td colspan="2" class='text-right' style="padding: 5px;"></td>
    ...
    <?php } ?>
    <tr>
      <td colspan="13" class='text-right'><b><?= $this->lang->line('total_item_vat'); ?></b></td>
      <td colspan="3" class='text-right'><b><?php echo store_number_format($total_in_tax); ?></b></td>
    </tr>
    ...
    <?php if ($tot_price_zero_vat != 0) { ?>
      <tr>
        <td colspan="13" class='text-right'><b><?= $this->lang->line('total_zero_vat'); ?></b></td>
        <td colspan="3" class='text-right'><b><?php echo store_number_format($total_tax_zero_amt); ?></b></td>
      </tr>
    <?php } ?>
        ...
        <table width="100%" class="style_hidden fixed_table">
          <tr>
            <td colspan="8">
              ...
              <span style="font-size: 13px;"><b><?php echo $quotation_note; ?></b></span>
              <br>
            <!--  <span>..............................................</span><br>
              <span>(............................................)</span><br>
              <span><?= $this->lang->line('customer_signature'); ?></span> -->
            </td>

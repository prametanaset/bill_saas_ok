                            ...
                            <div class="form-group">
                              ...
                              <div class="form-group">
                                 ...
                                 <div class="col-sm-3">
                                  ...
                                  <label for="reference_no" class="col-sm-2 control-label"><?= $this->lang->line('reason'); ?> </label>
                                 <div class="col-sm-3">
                                    <input type="text" value="<?php echo  $reference_no; ?>" class="form-control " id="reference_no" name="reference_no" placeholder="เหตุผลคืนสินค้า" >
        ...
        function calculate_tax(i){ //i=Row
         ...
         /* ---------- CALCULATE GST END -------------*/
          ...
          function final_total(){
            ...
            // Other charges (Note: input usually disabled in UI for returns but logic kept for consistency)
            var other_charges_input = parse_float($("#other_charges_input").val());
            var other_charges_total_amt = 0;
            
            // Prevailing VAT rate for charges (Priority: Store VAT > Max Item Rate)
            var prevailing_vat = max_tax_rate;
            if(prevailing_vat == 0 && typeof store_vat_no !== 'undefined') prevailing_vat = store_vat_no;
            ...
            if(other_charges_input > 0){
               ...
               if(oc_tax_rate <= 0) oc_tax_rate = prevailing_vat;
               
               if(oc_tax_rate > 0){
                  // Treat other charges as Inclusive for Sequence A standardization
            ...
            // Subtotal before any discount to all
            ...
            // Extract VAT from the global discount before spreading (Sync with POS/Sales)
            var total_global_discount_base = (total_discount * 100) / (100 + prevailing_vat);
            ...
            for(var rate in tax_groups){
               ...
               var vat_amt_row = Math.round((rounded_net_base * parse_float(rate) / 100) * 100) / 100;
               final_grand_total += rounded_net_base + vat_amt_row;
            }

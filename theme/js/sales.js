...
$('#save,#update').on("click", function (e) {
  ...
  if (flag == false) {
    toastr["error"]("คุณขาดข้อมูลบางอย่าง !");
    return;
  }
...
selectInitial :true,*/
      ...
      error: function () {
        toastr['error']("Failed to search items. Please try again.");
      },
      complete: function () {
        $("#item_search").removeClass('ui-autocomplete-loading');
      }
...
function restrict_quantity(item_id, service_bit = 0) {
  ...
  if (service_bit == 0 && available_qty != 0 && count_item_qty >= available_qty) {
    toastr["warning"]("เฉพาะ " + available_qty + " สินค้าในสต็อก!!");
    failed.currentTime = 0;
    failed.play();
    return false;
  }

...
$('#save,#update').on("click", function (e) {
  ...
  if (flag == false) {
    toastr["error"]("ขาดข้อมูลบางอย่าง!");
    return;
  }
  ...
  $.ajax({
    ...
    error: function (xhr, status, error) {
      toastr["error"]("Failed to save! Error: " + error);
...
function return_row_with_data(item_id) {
  ...
  $.ajax({
    type: 'POST',
    url: base_url + "quotation/return_row_with_data/" + rowcount + "/" + item_id,
    ...
    error: function (xhr, status, error) {
      toastr['error']("Failed to fetch item details! Try again.");
      $("#item_search").removeClass('ui-autocomplete-loader-center');
      $("#item_search").removeClass('ui-autocomplete-loading');
    }
  });

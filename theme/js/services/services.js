$('#save,#update').on("click", function (e) {
	...
	if (flag == false) {
		toastr["warning"]("ขาดข้อมูลบางอย่าง!");
		return;
	}
	...
	if (this_id == "save")  //Save start
	{
		...
		$.ajax({
			...
			error: function (xhr, status, error) {
				// Show the error response from the server (e.g., PHP Fatal Error)
				var errorMessage = xhr.responseText;
				// Strip HTML tags to make it readable in toastr which might interpret HTML or be too long
				var cleanError = errorMessage.replace(/<\/?[^>]+(>|$)/g, "");

				toastr["error"]("Server Error: " + cleanError.substring(0, 200));

...
$("#save,#update").on("click",function(){
      ...
      if (email!='' && !validateEmail(email)) {
          ...
          toastr["warning"]("Email ไม่ถูกต้อง !")
          return false;
      }
        ...
        //if(confirm("Are you sure ?")){
         ...
         $.post(base_url+"warehouse/save_or_update",{command:this_id,q_id:q_id,warehouse_name:warehouse_name,email:email,mobile:mobile,store_id:$("#store_id").val()},function(result){
            ...
            if(result=="success"){
                ...
                toastr["error"]("อัพเดทข้อมูล ไม่สำเร็จ พยายามอีกครั้ง !");
            }
            else
            {
                toastr["error"](result);
...
//alert(result);return;
        if(result=="success")
        {
          ...
          toastr["error"]("อัพเดท ไม่สำเร็จ.ลองอีกครั้ง!");
          failed.currentTime = 0; 
          failed.play();
        }
        else{
         toastr["error"]("Error! มีบางอย่างผิดปกติ!");
         failed.currentTime = 0; 
         failed.play();
        }
         return false;

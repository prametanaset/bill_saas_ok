...
$('#save,#update').on("click", function (e) {
  ...
  $.ajax({
    type: 'POST',
    url: base_url + 'subscribers/save_update_subscription?command=' + this_id,
    data: data,
    cache: false,
    contentType: false,
    processData: false,
...
function update_status(id, status) {
  var base_url = $("#base_url").val();
  $.post(base_url + "subscribers/update_status", { id: id, status: status }, function (result) {
    if (result == "success") {
...
function delete_subscription(q_id) {
  ...
  if (confirm("Do You Wants to Delete Record ?")) {
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    $.post(base_url + "subscribers/delete_subscription", { q_id: q_id }, function (result) {
      //alert(result);return;
      if (result == "success") {
...
function multi_delete() {
  ...
  if (confirm("Are you sure ?")) {
    ...
    $.ajax({
      type: 'POST',
      url: base_url + 'subscribers/multi_delete',
      data: data,
      cache: false,
      contentType: false,
      processData: false,

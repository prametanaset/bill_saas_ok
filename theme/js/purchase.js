...
$('#save,#update').on("click", function (e) {
  ...
  $.ajax({
    ...
    error: function (xhr, status, error) {
      ...
      toastr["error"]("เกิดข้อผิดพลาดในการบันทึกข้อมูล กรุณาลองใหม่อีกครั้ง");
...
function return_row_with_data(item_id) {
  ...
  $.ajax({
    type: 'POST',
    url: base_url + "purchase/return_row_with_data/" + rowcount + "/" + item_id,
    data: {},
    success: function (result) {
      //alert(result);
      ...
      try { enable_or_disable_item_discount(); } catch (e) { console.error("Discount Error:", e); }
      try { final_total(); } catch (e) { console.error("Final Total Error:", e); }
    },
    error: function (xhr, status, error) {
      console.error("AJAX Error: " + status + " - " + error);
      toastr["error"]("เกิดข้อผิดพลาดในการโหลดข้อมูลสินค้า กรุณาลองใหม่อีกครั้ง");
      try { failed.currentTime = 0; failed.play(); } catch (e) { }
    },
    complete: function () {

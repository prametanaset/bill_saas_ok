...
$(".add_item").click(function (e) {
	...
	if (flag == false) {
		toastr["warning"]("คุณ ป้อนข้อมูลไม่ครบ!");
		return;
	}
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (result == "success") {
					...
					$('#item-form')[0].reset();
					toastr["success"]("เพิ่มสินค้าใหม่!!");
...
$(".add_service").click(function (e) {
	...
	if (flag == false) {
		toastr["warning"]("คุณป้อนข้อมูลไม่ครบ !");
		return;
	}
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (result == "success") {
					...
					//$("#amount").val(data.advance);
					$('#item-form')[0].reset();
					toastr["success"]("เพิ่มสินค้า บริการใหม่!!");
					success_sound.currentTime = 0;
					success_sound.play();
				}
				else if (result == "failed") {
					toastr["error"]("บันทึกไม่ได้ .พยายามอีกครั้ง!");
					failed.currentTime = 0;
					failed.play();
				}
				else {
